...
describe('Admin API Integration Tests', () => {
  ...
  describe('GET /api/admin/:id', () => {
    ...
    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/admin/invalid-id-format')
        .expect(400);
    });
  });
  ...
  describe('DELETE /api/admin/:id', () => {
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/admin/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/admin/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/admin/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/admin/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/admin?name=${encodeURIComponent(maliciousInput)}`)
