...
describe('Aide API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/aide', () => {
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/aide/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/aide/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/aide/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/aide/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/aide', () => {
  ...
  describe('DELETE /api/aide/:id', () => {
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/aide/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/aide/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/aide/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/aide/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/aide?name=${encodeURIComponent(maliciousInput)}`)
