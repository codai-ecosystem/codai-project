...
export class FileSystemAdapter implements PersistenceAdapter {
  ...
  private fs: any; // Will be dynamically imported
  private path: any; // Will be dynamically imported
        ...
        try {
          ...
          if (fileExists) {
            ...
            await this.fs.copyFile(
              graphPath,
              this.path.join(this.storagePath, `${graph.id}_backup_${timestamp}.json`)
            );
          }
        } catch (backupError) {
  ...
  async load(graphId?: string): Promise<MemoryGraph | null> {
    try {
      ...
      if (!graphId) {
        ...
        const graphFiles: string[] = await this.fs.readdir(this.storagePath);
        if (graphFiles.length === 0) return null;

        // Get file stats and find most recent
        const statsPromises = graphFiles
          .filter((file: string) => file.endsWith('.json') && !file.includes('_backup_'))
      ...
      const graphPath = this.getGraphPath(graphId as string);
  ...
  async importGraph(data: string, format = 'json'): Promise<MemoryGraph | null> {
    try {
      ...
      if (format === 'json') {
        ...
        // Save to filesystem
        await this.fs.writeFile(
          this.getGraphPath(validatedGraph.id),
          JSON.stringify(validatedGraph, null, 2),
          'utf8'
        );
