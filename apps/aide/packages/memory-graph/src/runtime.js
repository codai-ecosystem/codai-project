...
export class AgentMemoryRuntime {
    ...
    logChange(change) {
        ...
        console.log(`[MemoryGraph Change] ${change.type}`, change.node?.type || change.relationship?.type || '', change.node?.name || '');
    }
    /**
    ...
    recordIntent(name, description, relatedToConversation = true) {
        ...
        if (relatedToConversation && this.currentConversationNodeId) {
            GraphBuilders.relationship(this.currentConversationNodeId, intentNode.id, 'derives_from').addToGraph(this.engine);
        }
        return intentNode;
    }
    /**
    ...
    recordFeature(name, description, status = 'planned', priority = 'medium', relatedIntentId) {
        const featureNode = GraphBuilders.feature(name)
            ...
            .addToGraph(this.engine);
        if (relatedIntentId) {
            GraphBuilders.relationship(relatedIntentId, featureNode.id, 'implements').addToGraph(this.engine);
        }
        return featureNode;
    }
    /**
    ...
    recordScreen(name, description, screenType = 'page', route, relatedFeatureId) {
        ...
        if (relatedFeatureId) {
            GraphBuilders.relationship(screenNode.id, relatedFeatureId, 'implements').addToGraph(this.engine);
        }
        return screenNode;
    }
    /**
     ...
     */
    recordLogic(name, description, logicType = 'function', relatedNodeId, relationshipType = 'implements') {
        const logicNode = GraphBuilders.logic(name)
            .withDescription(description)
            .withLogicType(logicType)
            .addToGraph(this.engine);
        if (relatedNodeId) {
            GraphBuilders.relationship(logicNode.id, relatedNodeId, relationshipType).addToGraph(this.engine);
        }
        return logicNode;
    }
    /**
