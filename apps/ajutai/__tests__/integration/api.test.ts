...
describe('Ajutai API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/ajutai', () => {
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/ajutai/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/ajutai/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/ajutai/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/ajutai/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/ajutai', () => {
  ...
  describe('PUT /api/ajutai/:id', () => {
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .put('/api/ajutai/non-existent-id')
  ...
  describe('DELETE /api/ajutai/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/ajutai/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/ajutai/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/ajutai/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/ajutai/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/ajutai/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/ajutai?name=${encodeURIComponent(maliciousInput)}`)
