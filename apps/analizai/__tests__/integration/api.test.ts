...
describe('Analizai API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/analizai', () => {
    ...
    it('should handle pagination parameters', async () => {
      ...
      const response = await request(app)
        .get('/api/analizai?page=2&limit=5')
        .expect(200);
    ...
    it('should handle search filters', async () => {
      ...
      const response = await request(app)
        .get('/api/analizai?status=active')
        .expect(200);
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/analizai/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/analizai/:id', () => {
    it('should return specific record by ID', async () => {
      ...
      const response = await request(app)
        .get(`/api/analizai/${testRecord.id}`)
        .expect(200);
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/analizai/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/analizai/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/analizai', () => {
  ...
  describe('PUT /api/analizai/:id', () => {
    it('should update record successfully', async () => {
      ...
      const response = await request(app)
        .put(`/api/analizai/${testRecord.id}`)
        .send(updateData)
        .expect(200);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .put('/api/analizai/non-existent-id')
  ...
  describe('DELETE /api/analizai/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/analizai/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/analizai/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/analizai/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/analizai/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/analizai/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/analizai?name=${encodeURIComponent(maliciousInput)}`)
