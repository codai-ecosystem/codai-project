...
import {
    ...
    Brain, Database, Activity, DollarSign, Eye, Clock, CheckCircle,
    XCircle, RefreshCw, Download, BarChart3, LineChart, PieChart as PieChartIcon,
    ArrowUp, ArrowDown, TrendingUp as TrendingUpIcon
} from 'lucide-react'
import {
    LineChart, AreaChart, BarChart, PieChart, Line, Area, Bar, Pie,
    XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, Cell
} from 'recharts'
import { AnalizaiService } from '../../../services/analizai-service'
...
interface AIInsight {
    id: string
    type: 'trend' | 'anomaly' | 'prediction' | 'alert' | 'recommendation'
    title: string
    description: string
    confidence: number
...
export default function AnalizaiDashboard() {
    ...
    useEffect(() => {
        const loadDashboardData = async () => {
            try {
                const [metricsData, insightsData, dataSourcesData, chartsData] = await Promise.all([
                    analizaiService.getMetrics(),
                    analizaiService.getInsights(),
                    analizaiService.getDataSources(),
                    analizaiService.getCharts()
                ])
