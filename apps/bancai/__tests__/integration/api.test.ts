...
describe('Bancai API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/bancai', () => {
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/bancai/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/bancai/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/bancai/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/bancai/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/bancai', () => {
  ...
  describe('PUT /api/bancai/:id', () => {
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .put('/api/bancai/non-existent-id')
  ...
  describe('DELETE /api/bancai/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/bancai/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/bancai/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/bancai/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/bancai/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/bancai/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/bancai?name=${encodeURIComponent(maliciousInput)}`)
