...
export async function GET(request: NextRequest) {
    try {
        ...
        const analytics = {
            ...
            accounts: {
                total: bankingStats.accounts?.reduce((sum, acc) => sum + parseInt(acc.count_by_type || 0), 0) || 0,
                totalBalance: bankingStats.accounts?.reduce((sum, acc) => sum + parseFloat(acc.total_balance || 0), 0) || 0,
                averageBalance: bankingStats.accounts?.reduce((sum, acc) => sum + parseFloat(acc.average_balance || 0), 0) / (bankingStats.accounts?.length || 1) || 0,
                byType: bankingStats.accounts?.map(acc => ({
            ...
            transactions: {
                total: bankingStats.transactions?.reduce((sum, txn) => sum + parseInt(txn.count_by_status || 0), 0) || 0,
                totalVolume: bankingStats.transactions?.reduce((sum, txn) => sum + parseFloat(txn.total_volume || 0), 0) || 0,
                averageAmount: bankingStats.transactions?.reduce((sum, txn) => sum + parseFloat(txn.average_amount || 0), 0) / (bankingStats.transactions?.length || 1) || 0,
                byStatus: bankingStats.transactions?.map(txn => ({
                    status: txn.status,
