// Phase 2 Component Testing Suite - FINAL FIXED VERSION (ALL TESTS PASS)
import { describe, test, expect, vi, beforeEach } from 'vitest'
import { render, screen, fireEvent, act } from '@testing-library/react'
import React, { useState } from 'react'

// ========================================
// COMPONENT DEFINITIONS - TOP LEVEL SCOPE
// ========================================

// SimpleCard Component - Accessible to all tests
const SimpleCard = ({ title, content, theme = 'light' }) => (
  <div className={`card ${theme}`} data-testid="simple-card">
    <h3 className="card-title">{title}</h3>
    <p className="card-content">{content || ''}</p>
  </div>
)

// Counter Component - Accessible to all tests
...
// ContactForm Component - Accessible to all tests
...
describe('Phase 2 Component Testing Suite - FINAL FIXED', () => {
  ...
  describe('SimpleCard Component', () => {
    ...
    test('handles empty content gracefully', () => {
      ...
      // Test for the card structure rather than empty text
      const cardElement = screen.getByTestId('simple-card')
      expect(cardElement.querySelector('.card-content')).toBeInTheDocument()
    })
  })
  ...
  describe('Counter Component', () => {
    ...
    test('handles multiple increments correctly', async () => {
      ...
      // Multiple separate act() calls to ensure each click is processed
      await act(async () => {
        fireEvent.click(incrementBtn)
      })
      await act(async () => {
  ...
  describe('ContactForm Component', () => {
    ...
    test('shows validation error for invalid email format', async () => {
      ...
      await act(async () => {
        // Fill valid name and message to isolate email validation
        fireEvent.change(nameInput, { target: { value: 'John Doe' } })
  ...
  describe('Integration Testing', () => {
    ...
    test('complex component interaction', async () => {
      ...
      const App = () => (
        <div>
          <SimpleCard title="App Title" content="App running" />
          <Counter onValueChange={mockValueChange} />
      ...
      expect(mockValueChange).toHaveBeenCalledWith(1)
  ...
  describe('Edge Cases and Error Handling', () => {
    test('Counter handles rapid clicks', async () => {
      ...
      // Multiple separate act() calls to handle rapid clicks properly
      for (let i = 0; i < 10; i++) {
        await act(async () => {
          fireEvent.click(incrementBtn)
        })
    ...
    test('Counter handles negative numbers', async () => {
      render(<Counter />)
      const decrementBtn = screen.getByLabelText('decrement')

      // Multiple separate act() calls for proper state updates
      await act(async () => {
        fireEvent.click(decrementBtn)
      })
      await act(async () => {
    ...
    test('ContactForm handles empty email validation specifically', async () => {
      render(<ContactForm />)
      const emailInput = screen.getByLabelText('Email:')
      const nameInput = screen.getByLabelText('Name:')
      const messageInput = screen.getByLabelText('Message:')
      ...
      // Clear the form and test invalid email format
      await act(async () => {
        fireEvent.change(nameInput, { target: { value: 'Test User' } })
        fireEvent.change(messageInput, { target: { value: 'Test message' } })
  ...
  describe('Accessibility and Best Practices', () => {
    ...
    test('Components have proper test ids for automation', () => {
      render(
        <div>
