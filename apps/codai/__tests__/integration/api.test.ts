...
describe('Codai API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/codai', () => {
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/codai/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/codai/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/codai/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/codai/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/codai', () => {
  ...
  describe('DELETE /api/codai/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/codai/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/codai/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/codai/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/codai/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/codai/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/codai?name=${encodeURIComponent(maliciousInput)}`)
