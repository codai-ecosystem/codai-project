...
import { MessageCircle, Code, BarChart3, Zap, Shield, Globe, Activity, Users, Server, Database } from 'lucide-react';
...
export default function Dashboard() {
	...
	const fetchRealTimeData = useCallback(async () => {
		try {
			// Check running services by port scanning
			const runningPorts: number[] = [];

			// Check app ports (4030-4040)
			for (let port = 4030; port <= 4040; port++) {
				try {
			...
			for (let port = 4001; port <= 4029; port++) {
				try {
			...
			setMetrics(prev => ({
				...prev,
				appsRunning: runningPorts.filter(p => p >= 4030).length,
				servicesRunning: runningPorts.filter(p => p <= 4029).length,
				systemHealth: Math.min(95, (runningPorts.length / 40) * 100),
			...
			setRealTimeStats({
				...
				activeConnections: Math.random() * 500 + 200,
	...
	useEffect(() => {
		...
		const fetchDashboardData = async () => {
			...
			// Enhanced mock services with more realistic data
		...
		const interval = setInterval(() => {
			fetchRealTimeData();
			setAnimationClass('animate-pulse-grow');
