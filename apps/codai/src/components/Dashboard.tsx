...
export default function Dashboard() {
	...
	const filteredServices =
		selectedTier === 'all' ? services : services.filter(service => service.tier === selectedTier);

	const totalRequests = services.reduce((sum, service) => sum + service.requests, 0);
	const averageUptime =
		services.length > 0
			? (
					services.reduce((sum, service) => sum + parseFloat(service.uptime), 0) / services.length
				).toFixed(1)
			: '0.0';
							...
							>
								...
								<div className="space-y-2 text-sm">
									...
									<div className="flex justify-between items-center">
										...
										<span
											className={`px-2 py-1 rounded-full text-xs font-medium border ${getTierColor(service.tier)}`}
										>
											{service.tier}
										</span>
									</div>
