...
describe('CODAI Service - E2E Tests', () => {
	let browser: Browser;
	let page: Page;
	...
	describe('Application Loading', () => {
		it('should load the main page successfully', async () => {
			await page.goto(baseUrl);   // @ts-ignore
			await expect(page).toHaveTitle(new RegExp(serviceName, 'i'));
	...
	describe('Core Functionality', () => {
		it('should perform main business operations', async () => {
			...
			await expect(page.locator('[data-testid="operation-success"]')).toBeVisible(); await expect(page.locator('[data-testid="operations-list"]')).toContain('Test Operation');
		});
		...
		it('should handle data persistence', async () => {
			...
			await page.reload();
			await expect(page.locator('[data-testid="data-list"]')).toContain('Test Data');
		});
		...
		it('should handle real-time updates', async () => {
			...
			await expect(page2.locator('[data-testid="updates-list"]')).toContain('Real-time test');
