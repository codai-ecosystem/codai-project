...
export const CleanHero: React.FC = () => {
    ...
    return (
        ...
        >
            ...
            <div
                className={`absolute inset-0 opacity-5 ${theme === 'dark' ? 'bg-white' : 'bg-gray-900'
                    }`}
                style={{
            ...
            {/* Main Hero Content */}
            <div className={`relative z-10 text-center max-w-7xl mx-auto transition-all duration-500 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                }`}>
                ...
                <header className="mb-8 sm:mb-12">
                    ...
                    <div className={`text-lg sm:text-xl md:text-2xl lg:text-3xl font-medium ${theme === 'dark' ? 'text-text-secondary' : 'text-gray-700'
                        }`}>
                ...
                <div className="mb-12 sm:mb-16 max-w-4xl mx-auto">
                    <p className={`text-lg sm:text-xl md:text-2xl font-medium leading-relaxed ${theme === 'dark' ? 'text-text-tertiary' : 'text-gray-600'
                        }`}>
                    ...
                    {[
                        ...
                        >
                            <div className="mb-2 sm:mb-3 flex justify-center">
                                <Icon
                                    className={`w-6 h-6 sm:w-8 sm:h-8 ${theme === 'dark' ? 'text-primary-400' : 'text-primary-600'
                                        }`}
                                    aria-hidden="true"
                                />
                            </div>
                            ...
                            <div
                                ...
                                className={`text-xl sm:text-2xl lg:text-3xl font-bold mb-1 sm:mb-2 font-display ${theme === 'dark' ? 'text-text-primary' : 'text-gray-900'
                                    }`}
                            >
                                {number}
                            </div>

                            <div className={`text-sm sm:text-base font-semibold mb-1 ${theme === 'dark' ? 'text-text-secondary' : 'text-gray-700'
                                } uppercase tracking-wide`}>
                                {label}
                            </div>

                            <div className={`text-xs sm:text-sm ${theme === 'dark' ? 'text-text-tertiary' : 'text-gray-500'
                                }`}>
                                {description}
                            </div>
                        </div>
                ...
                <div
                    className={`cursor-pointer transition-all duration-300 hover:scale-110 focus:scale-110 focus:outline-none focus:ring-4 focus:ring-primary-400/50 rounded-lg p-2 ${hasInteracted ? 'opacity-50' : ''
                        }`}
                    onClick={scrollToProjects}
                    onKeyDown={(e) => handleKeyPress(e, scrollToProjects)}
                    role="button"
                ...
                >
                    <div className={`flex flex-col items-center animate-fade-in-up ${theme === 'dark' ? 'text-text-tertiary hover:text-text-secondary' : 'text-gray-500 hover:text-gray-700'
                        }`}>
                        <ChevronDown
                            className={`w-6 h-6 mb-2 transition-transform ${hasInteracted ? '' : 'animate-bounce'
                                }`}
                            aria-hidden="true"
                        />
