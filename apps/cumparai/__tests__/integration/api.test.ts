...
describe('Cumparai API Integration Tests', () => {
  ...
  describe('GET /api/cumparai', () => {
    ...
    it('should handle pagination parameters', async () => {
      ...
      const response = await request(app)
        .get('/api/cumparai?page=2&limit=5')
        .expect(200);
    ...
    it('should handle search filters', async () => {
      ...
      const response = await request(app)
        .get('/api/cumparai?status=active')
        .expect(200);
  ...
  describe('GET /api/cumparai/:id', () => {
    it('should return specific record by ID', async () => {
      ...
      const response = await request(app)
        .get(`/api/cumparai/${testRecord.id}`)
        .expect(200);
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/cumparai/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/cumparai/invalid-id-format')
        .expect(400);
    });
  });
  ...
  describe('PUT /api/cumparai/:id', () => {
    it('should update record successfully', async () => {
      ...
      const response = await request(app)
        .put(`/api/cumparai/${testRecord.id}`)
        .send(updateData)
        .expect(200);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .put('/api/cumparai/non-existent-id')
  ...
  describe('DELETE /api/cumparai/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/cumparai/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/cumparai/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/cumparai/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/cumparai/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/cumparai/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/cumparai?name=${encodeURIComponent(maliciousInput)}`)
