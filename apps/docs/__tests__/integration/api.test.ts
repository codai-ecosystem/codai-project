...
describe('Docs API Integration Tests', () => {
  ...
  describe('GET /api/docs/:id', () => {
    ...
    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/docs/invalid-id-format')
        .expect(400);
    });
  });
  ...
  describe('DELETE /api/docs/:id', () => {
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/docs/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/docs/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/docs/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/docs?name=${encodeURIComponent(maliciousInput)}`)
