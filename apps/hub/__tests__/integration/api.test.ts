...
describe('Hub API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/hub', () => {
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/hub/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/hub/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/hub/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/hub/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/hub', () => {
  ...
  describe('DELETE /api/hub/:id', () => {
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/hub/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/hub/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/hub/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/hub/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/hub?name=${encodeURIComponent(maliciousInput)}`)
