...
describe('Id API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/id/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/id/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/id/invalid-id-format')
        .expect(400);
    });
  });
  ...
  describe('DELETE /api/id/:id', () => {
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/id/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/id/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/id/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/id?name=${encodeURIComponent(maliciousInput)}`)
