...
describe('Kodex API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/kodex', () => {
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/kodex/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/kodex/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/kodex/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/kodex/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/kodex', () => {
  ...
  describe('DELETE /api/kodex/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/kodex/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/kodex/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/kodex/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/kodex/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/kodex/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/kodex?name=${encodeURIComponent(maliciousInput)}`)
