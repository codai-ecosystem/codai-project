...
describe('Logai API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/logai', () => {
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/logai/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/logai/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/logai/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/logai/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/logai', () => {
  ...
  describe('DELETE /api/logai/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/logai/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/logai/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/logai/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/logai/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/logai/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/logai?name=${encodeURIComponent(maliciousInput)}`)
