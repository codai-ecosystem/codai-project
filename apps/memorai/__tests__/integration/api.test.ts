...
describe('Memorai API Integration Tests', () => {
  ...
  describe('GET /api/memorai', () => {
    ...
    it('should handle pagination parameters', async () => {
      ...
      const response = await request(app)
        .get('/api/memorai?page=2&limit=5')
        .expect(200);
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/memorai/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/memorai/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/memorai/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/memorai/invalid-id-format')
        .expect(400);
    });
  });
  ...
  describe('PUT /api/memorai/:id', () => {
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .put('/api/memorai/non-existent-id')
  ...
  describe('DELETE /api/memorai/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/memorai/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/memorai/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/memorai/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/memorai/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/memorai/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/memorai?name=${encodeURIComponent(maliciousInput)}`)
