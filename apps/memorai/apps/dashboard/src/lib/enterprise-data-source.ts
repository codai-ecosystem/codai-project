...
class EnterpriseDataSourceManager {
        ...
        api: {
          baseUrl:
            ...
            (typeof window !== 'undefined'
              ...
              : '/api'), // Fixed: Use relative URL to prevent port issues
  ...
  private async getSystemHealth() {
    ...
    // Fixed: Improved health check thresholds for development environment
    ...
    // Don't mark as critical just because of short uptime in development
    if (memoryPercentage > 98 && uptime < 30) {
      status = 'critical';
    }
  ...
  private calculateMemoryDistribution(
    ...
    const distribution = memories.reduce(
      (acc: Record<string, number>, memory: any) => {
  ...
  private async fetchMemoriesFromAPI() {
    try {
      console.log('🔍 Fetching memories from mcpMemoryClient directly...');

      // Use mcpMemoryClient directly instead of fetch
      // Server-side relative URLs don't work, so use the client directly
      const { mcpMemoryClient } = await import('./mcp-memory-client');

      // Get memories via mcpMemoryClient with enterprise query
      const memories = await mcpMemoryClient.searchMemories('', {
        limit: 100,
        agentId: 'system'
      });

      console.log(`✅ Successfully fetched ${memories.length} memories via mcpMemoryClient`);
      return memories;
    } catch (error) {
      console.error('❌ Error fetching memories via mcpMemoryClient:', error);
