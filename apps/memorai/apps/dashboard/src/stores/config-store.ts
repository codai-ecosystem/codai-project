...
const api = {
  async getConfig(): Promise<SystemConfig> {
    console.log('Config store: getConfig() called - using default config only');
    // For now, disable API calls and use default config only to avoid 500 errors
    // TODO: Re-enable when API routes are fixed
    /*
    try {
      // Try real API first
    ...
    */
    ...
    // Fallback to default config (currently the only path to avoid API errors)
    console.log('Config store: returning default config');
