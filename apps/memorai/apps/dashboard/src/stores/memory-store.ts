...
const api = {
  ...
  async getStats(): Promise<MemoryStats> {
    console.log('Memory store API: getStats() called - START');
    try {
      console.log('Memory store API: Calling mcpMemoryClient.getStats()...');
      const result = await mcpMemoryClient.getStats();
      console.log('Memory store API: mcpMemoryClient.getStats() returned:', JSON.stringify(result, null, 2));
      return result;
    } catch (error) {
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        fetchStats: async () => {
          console.log('Memory store: fetchStats() called - STARTING...');
          set({ isLoading: true, error: null });
          try {
            console.log('Memory store: Fetching stats...');
            const stats = await api.getStats();
            console.log('Memory store: Stats received:', JSON.stringify(stats, null, 2));
            console.log(`Memory store: Setting totalMemories to ${stats.totalMemories}`);
            set({ stats, isLoading: false });
            console.log('Memory store: fetchStats() completed successfully');
          } catch (error) {
