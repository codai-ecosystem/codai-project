...
// Mock the entire EmbeddingService module to focus on interface testing
vi.mock('../../src/embedding/EmbeddingService.js', () => ({
  OpenAIEmbeddingProvider: vi.fn().mockImplementation((config: any) => {
    if (!config.api_key) {
  ...
  EmbeddingService: vi.fn().mockImplementation((config: any) => {
    return {
      ...
      embedBatch: vi.fn().mockImplementation((texts: string[]) => {
        const validTexts = texts.filter((text: string) => text && text.trim() !== '');
      ...
      embedWithRetry: vi.fn().mockImplementation((text: string) => {
        if (!text || text.trim() === '') {
          throw new Error('Text cannot be empty');
        }
        return {
...
describe('EmbeddingService', () => {
  ...
  describe('OpenAIEmbeddingProvider', () => {
    const mockConfig: MemoryConfig['embedding'] = {
      ...
      endpoint: 'https://test.openai.azure.com/openai/deployments/text-embedding-3-small',
