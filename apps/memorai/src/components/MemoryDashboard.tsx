...
export default function MemoryDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    'memories' | 'connections' | 'analytics'
  >('memories');
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await memoryService.query({
        ...
        type:
          selectedType !== 'all'
            ? (selectedType as MemoryEntity['type'])
            : undefined,
        limit: 50,
      });
  ...
  const filteredMemories = memories.filter((memory: MemoryEntity) => {
    ...
    if (selectedAgent !== 'all' && memory.agentId !== selectedAgent)
      return false;
    if (
      searchQuery &&
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-6 rounded-lg">
          ...
          <p className="text-purple-100">
            AI Memory & Database Core - Model Context Protocol Server
          </p>
        </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">
                  {stats.queryCount.toLocaleString()}
                </p>
              </div>
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-medium ${getTypeColor(memory.type)}`}
                            >
                              {memory.type}
                            </span>
                            {memory.agentId && (
                  ...
                  {connections.map(connection => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span
                          className={`font-medium ${getConnectionStatusColor(connection.status)}`}
                        >
                          {connection.status}
                        </span>
                      </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        const percentage =
                          memories.length > 0
                            ? ((count / memories.length) * 100).toFixed(1)
                            : '0';
                        return (
                          <div
                            key={type}
