...
export class MCPServer {
    ...
    public async handleConnection(agentId: string, endpoint: string): Promise<MCPConnection> {
    ...
    public async handleQuery(connectionId: string, request: QueryRequest): Promise<any> {
    ...
    public async handleRemember(connectionId: string, request: MemoryRequest): Promise<any> {
        const connection = this.connections.get(connectionId)
    ...
    public async handleForget(connectionId: string, memoryId: string): Promise<any> {
        const connection = this.connections.get(connectionId)
    ...
    public getServerStatus(): any {
        return {
            ...
            totalQueries: Array.from(this.connections.values()).reduce((sum, c) => sum + c.queryCount, 0)
        }
    }
}
