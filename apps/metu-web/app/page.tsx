...
import type { VoiceMessage, VoiceState } from '../src/services/VoiceService';
...
export default function MetuVoiceAI() {
  ...
  // Sync app settings with voice settings
  useEffect(() => {
    updateVoiceSettings({
  ...
  }, [appSettings, updateVoiceSettings]);
  ...
  }, [voiceState, appSettings.voiceEnabled, isSupported, startListening, stopListening, stopSpeaking]);
  ...
  const handleTestSpeak = useCallback(async () => {
    try {
      await speak('Hello! I am METU, your voice-powered AI assistant. How can I help you today?');
    } catch (error) {
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`w-12 h-6 rounded-full transition-colors ${appSettings.voiceEnabled ? 'bg-primary-500' : 'bg-slate-600'
                        }`}
                    >
                      <div className={`w-5 h-5 bg-white rounded-full transition-transform ${appSettings.voiceEnabled ? 'translate-x-6' : 'translate-x-0.5'
                        }`} />
                    </button>
                  </div>
