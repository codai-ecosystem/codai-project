...
try {
    ...
    Write-Host " Security vulnerabilities found. Please review before deploying." -ForegroundColor Yellow
}
...
try {
    ...
    if ($response.status -eq "healthy") {
        ...
        Write-Host " Main server health check warning: $($response.status)" -ForegroundColor Yellow
    }
} catch {
...
try {
    ...
    if ($response.status -eq "healthy") {
        ...
        Write-Host " Device server health check warning: $($response.status)" -ForegroundColor Yellow
    }
} catch {
...
if ($healthChecksPassed -eq $totalHealthChecks) {
    ...
    Write-Host " METU Deployment Completed with Issues" -ForegroundColor Yellow
