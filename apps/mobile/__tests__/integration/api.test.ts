...
describe('Mobile API Integration Tests', () => {
  ...
  describe('GET /api/mobile', () => {
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/mobile/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/mobile/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/mobile/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/mobile/invalid-id-format')
        .expect(400);
    });
  });
  ...
  describe('PUT /api/mobile/:id', () => {
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .put('/api/mobile/non-existent-id')
  ...
  describe('DELETE /api/mobile/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/mobile/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/mobile/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/mobile/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/mobile/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/mobile/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/mobile?name=${encodeURIComponent(maliciousInput)}`)
