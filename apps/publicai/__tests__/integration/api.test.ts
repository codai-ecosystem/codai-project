...
describe('Publicai API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/publicai', () => {
    ...
    it('should handle pagination parameters', async () => {
      ...
      const response = await request(app)
        .get('/api/publicai?page=2&limit=5')
        .expect(200);
    ...
    it('should handle search filters', async () => {
      ...
      const response = await request(app)
        .get('/api/publicai?status=active')
        .expect(200);
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/publicai/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/publicai/:id', () => {
    it('should return specific record by ID', async () => {
      ...
      const response = await request(app)
        .get(`/api/publicai/${testRecord.id}`)
        .expect(200);
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/publicai/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/publicai/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/publicai', () => {
  ...
  describe('PUT /api/publicai/:id', () => {
    it('should update record successfully', async () => {
      ...
      const response = await request(app)
        .put(`/api/publicai/${testRecord.id}`)
        .send(updateData)
        .expect(200);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .put('/api/publicai/non-existent-id')
  ...
  describe('DELETE /api/publicai/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/publicai/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/publicai/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/publicai/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/publicai/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/publicai/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/publicai?name=${encodeURIComponent(maliciousInput)}`)
