...
class PerformanceOptimizationServer {
    constructor(port = 8767) {
        ...
        this.optimizer = new performance_optimizer_1.PerformanceOptimizer({
    ...
    setupOptimizerEvents() {
        ...
        this.optimizer.on('bottlenecks', (reports) => {
            this.broadcastToSubscribers('performance', {
                ...
                timestamp: new Date().toISOString(),
                id: this.generateMessageId(),
                severity: reports.reduce((max, r) => r.severity === 'critical' ? 'critical' :
    ...
    broadcastToSubscribers(subscription, message) {
        for (const client of this.clients.values()) {
            if (client.subscriptions[subscription] && client.ws.readyState === ws_1.default.OPEN) {
...
exports.default = PerformanceOptimizationServer;
