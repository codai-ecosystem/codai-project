...
class CulturalMemoryBank(nn.Module):
    ...
    def __init__(self, config: MemoryConfig):
        ...
        self.semantic_memory = nn.Parameter(
            torch.randn(config.memory_bank_size, config.memory_dim) * 0.02
        )
        self.episodic_memory = nn.Parameter(
            torch.randn(config.memory_bank_size, config.memory_dim) * 0.02
        )
        self.cultural_memory = nn.Parameter(
            torch.randn(config.memory_bank_size, config.cultural_memory_dim) * 0.02
        )
...
class MemoryAttentionLayer(nn.Module):
    ...
    def __init__(self, config: MemoryConfig):
        ...
        if config.temporal_attention:
            self.temporal_encoding = nn.Parameter(torch.randn(1000, self.d_model) * 0.02)
