...
function Test-NginxConfiguration {
    ...
    if ($sslConfig.Count -gt 0) {
        ...
        Write-Host "    ✅ SSL/TLS configuration: PRESENT" -ForegroundColor Green
    } else {
        $TEST_RESULTS += @{
            Test = "SSL_TLS_Config"
            Status = "WARN"
    ...
    if ($foundHeaders -ge 4) {
        $TEST_RESULTS += @{
            ...
            Status = "PASS"
            Details = "$foundHeaders security headers configured"
        }
        Write-Host "    ✅ Security headers: $foundHeaders configured" -ForegroundColor Green
    } else {
        $TEST_RESULTS += @{
            Test = "Security_Headers"
            Status = "WARN"
            Details = "Only $foundHeaders security headers configured"
        }
        Write-Host "    ⚠️  Security headers: Only $foundHeaders configured" -ForegroundColor Yellow
    }
}
