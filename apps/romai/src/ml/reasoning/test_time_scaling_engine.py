...
class TestTimeScalingEngine:
    """🚀 Master Test-Time Scaling & Reasoning Engine with GPT-5 Style Compute Scaling"""
    ...
    def __init__(self):
        ...
        # Import the advanced test-time scaler
        try:
            from .advanced_test_time_scaling import AdvancedTestTimeScaler, ComputeScalingStrategy
            self.advanced_scaler = AdvancedTestTimeScaler()
            self.has_advanced_scaling = True
            logger.info("🧠 Advanced GPT-5 style test-time compute scaling enabled")
        except ImportError:
            self.advanced_scaler = None
            self.has_advanced_scaling = False
            logger.warning("Advanced test-time scaling not available - using basic scaling")
        
        logger.info("🚀 Test-Time Scaling Engine initialized - Advanced reasoning ready")
    
    async def solve_with_compute_scaling(
        self,
        problem: str,
        max_iterations: Optional[int] = None,
        target_confidence: float = 0.95,
        strategy: str = "gpt5_thinking",
        domain: str = "general"
    ) -> Union[ChainOfThoughtResult, Any]:
        """
        🧠 Solve problem using advanced GPT-5 style test-time compute scaling
        
        Args:
            problem: Problem to solve
            max_iterations: Maximum reasoning iterations (1-50)
            target_confidence: Target confidence level for early stopping
            strategy: Scaling strategy ("gpt5_thinking", "confidence_based", etc.)
            domain: Problem domain for specialized reasoning
        
        Returns:
            Advanced test-time compute result or fallback to basic scaling
        """
        
        if self.has_advanced_scaling:
            try:
                # Use advanced GPT-5 style compute scaling
                from .advanced_test_time_scaling import ComputeScalingStrategy
                
                # Map string strategies to enums
                strategy_mapping = {
                    "gpt5_thinking": ComputeScalingStrategy.GPT5_THINKING,
                    "confidence_based": ComputeScalingStrategy.CONFIDENCE_BASED,
                    "complexity_adaptive": ComputeScalingStrategy.COMPLEXITY_ADAPTIVE,
                    "uncertainty_reduction": ComputeScalingStrategy.UNCERTAINTY_REDUCTION,
                    "multi_path_consensus": ComputeScalingStrategy.MULTI_PATH_CONSENSUS,
                    "fixed_iterations": ComputeScalingStrategy.FIXED_ITERATIONS
                }
                
                compute_strategy = strategy_mapping.get(strategy, ComputeScalingStrategy.GPT5_THINKING)
                
                # Execute advanced test-time compute scaling
                result = await self.advanced_scaler.scale_reasoning(
                    problem=problem,
                ...
                logger.info(f"✅ Advanced scaling complete: {result.total_iterations} iterations, confidence: {result.final_confidence:.3f}")
                return result
                
            except Exception as e:
                logger.error(f"❌ Advanced scaling failed: {e}")
                # Fall back to basic scaling
        
        # Fallback to basic scaling if advanced is not available
        return await self.solve_with_scaling(problem, ReasoningStrategy.AUTO, domain)
