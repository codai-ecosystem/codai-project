...
class RomAITransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.layers = nn.ModuleList([
            RomAIHybridLayer(d_model=d_model, n_heads=n_heads)
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, return_hidden_states: bool = False) -> torch.Tensor:
        ...
        for layer in self.layers:
            x, context_state = layer(x, context_state, attention_mask)
        
        # Output projection
        x = self.output_norm(x)
        
        if return_hidden_states:
            return x  # Return hidden states for multimodal fusion
...
if __name__ == "__main__":
    ...
    print("Testing RomAI Hybrid Architecture...")
    
    # Create model
    model = RomAITransformer(
        vocab_size=30000,
        d_model=512,
        num_layers=6,
        n_heads=8,
    ...
    with torch.no_grad():
        ...
        print("✅ RomAI Hybrid Architecture test passed!")
