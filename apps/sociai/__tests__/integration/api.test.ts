...
describe('Sociai API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/sociai', () => {
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/sociai/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/sociai/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/sociai/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/sociai/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/sociai', () => {
  ...
  describe('PUT /api/sociai/:id', () => {
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .put('/api/sociai/non-existent-id')
  ...
  describe('DELETE /api/sociai/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/sociai/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/sociai/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/sociai/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/sociai/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/sociai/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/sociai?name=${encodeURIComponent(maliciousInput)}`)
