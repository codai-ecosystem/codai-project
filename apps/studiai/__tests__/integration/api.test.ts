...
describe('Studiai API Integration Tests', () => {
  let app: any;
  ...
  describe('GET /api/studiai', () => {
    ...
    it('should handle pagination parameters', async () => {
      ...
      const response = await request(app)
        .get('/api/studiai?page=2&limit=5')
        .expect(200);
    ...
    it('should return 404 for non-existent endpoint', async () => {
      await request(app)
        .get('/api/studiai/non-existent')
        .expect(404);
    });
  });

  describe('GET /api/studiai/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/studiai/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/studiai/invalid-id-format')
        .expect(400);
    });
  });

  describe('POST /api/studiai', () => {
  ...
  describe('PUT /api/studiai/:id', () => {
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .put('/api/studiai/non-existent-id')
  ...
  describe('DELETE /api/studiai/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/studiai/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/studiai/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/studiai/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/studiai/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/studiai/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/studiai?name=${encodeURIComponent(maliciousInput)}`)
