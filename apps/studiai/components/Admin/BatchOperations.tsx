...
const BatchOperations: React.FC = () => {
  ...
  const handleBatchOperation = async () => {
    ...
    try {
          ...
          case 'price':
            if (contentType === 'course') {
              batch.update(itemRef, {
                price: priceValue === '0' ? 0 : parseInt(priceValue),
              });
            }
            break;
        }
      ...
      setSelectAll(false);
      setSuccess(
        `Successfully applied ${selectedAction} to ${selectedItems.length} items`
      );
  ...
  if (!isAdmin) {
    return (
      <div className="p-4">You don't have permission to access this page.</div>
    );
  }
    ...
    <div className="space-y-6">
                ...
                <div className="flex gap-3">
                  {actionOptions.map(action => (
                    <Button
                      key={action.value}
                      color={
                        selectedAction === action.value ? 'primary' : 'default'
                      }
                      onPress={() =>
                        setSelectedAction(action.value as BatchAction)
                      }
                      isDisabled={
                        action.value === 'price' && contentType !== 'course'
                      }
                    >
                      {action.label}
                    </Button>
      ...
      >
        <ModalContent>
          ...
          <ModalBody>
            Are you sure you want to delete {selectedItems.length} {contentType}
