...
const CourseContent: React.FC<CourseContentProps> = ({
  ...
  const validLessons = Array.isArray(lessons)
    ? lessons.filter(lesson => lesson != null)
    : [];
  ...
  if (sortedLessons.length === 0) {
    console.warn(
      `No lessons to display for course: ${course?.id || 'unknown'}`,
      {
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-sm text-[color:var(--ai-muted)]">
                ...
                {sortedLessons.length === 1 ? 'lesson' : 'lessons'} completed
              </p>
            </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-[color:var(--ai-muted)] mt-1">
                Purchase this course to unlock all lessons. Free preview lessons
        ...
        {modules.length > 0 ? (
          ...
          <>
            ...
            {lessonsByModule['default'] &&
              lessonsByModule['default'].length > 0 && (
                <motion.div
                  variants={itemVariants}
                  className="border border-[color:var(--ai-card-border)] rounded-xl overflow-hidden bg-[color:var(--ai-card-bg)] shadow-sm"
                >
                ...
                {sortedLessons.map((lesson, index) => {
                  ...
                  return (
                    <LessonItem
                      ...
                      isAccessible={isLessonAccessible(lesson)}
                      onClick={() =>
                        isLessonAccessible(lesson) && handleLessonClick(lesson)
                      }
                    />
                  );
                })}
