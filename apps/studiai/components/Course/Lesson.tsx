...
export default function Lesson({ lesson, onClose }: LessonProps) {
  ...
  useEffect(() => {
    if (courseId && lessons[courseId]) {
      const courseLessons = Object.values(lessons[courseId]).sort(
      ...
      const currentIndex = courseLessons.findIndex(
        (l: LessonInterface) => l.id === lesson.id
      );
  ...
  const saveCurrentProgress = useCallback(() => {
    ...
    const videoProgress = videoRef.current
      ...
      : 0; // Pass position as the third parameter (videoProgress as a number)
  ...
  const calculateProgress = () => {
    ...
    const sortedLessons = Object.values(lessons[courseId]).sort(
      (a: LessonInterface, b: LessonInterface) => {
        const orderA = a.order || 0;
        const orderB = b.order || 0;
        return orderA - orderB;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="border border-[color:var(--ai-card-border)] bg-[color:var(--ai-card-bg)]/50 backdrop-blur-sm shadow-md rounded-2xl overflow-hidden w-full">
            <div className="p-0">
              <LessonNavigation
                ...
                onClose={onClose}
                lessons={Object.values(lessons[courseId] || {})
              ...
              <div className="p-5">
                ...
                <div className="flex items-center justify-between text-xs text-[color:var(--ai-muted)]">
                  <span>
                    Lesson{' '}
                    {calculateProgress() /
                      (100 / Object.keys(lessons[courseId] || {}).length)}{' '}
                    of {Object.keys(lessons[courseId] || {}).length}
                  </span>
                  <span>
                    {Math.round(calculateProgress())}% of course completed
                  </span>
                </div>
              </div>
            </Card>
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          {lesson.resources && lesson.resources.length > 0 && (
            <LessonResources
              resources={lesson.resources.map((r: LessonResource) => ({
                name: r.name || r.title || '',
                url: r.url || '',
                type: r.type,
