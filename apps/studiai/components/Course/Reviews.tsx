...
export default function Reviews({
  ...
  const getReviews = async () => {
    ...
    try {
      const q = query(
        collection(firestoreDB, 'courses', courseId as string, 'reviews')
      );
  ...
  const handleReview = async (e: any) => {
    ...
    try {
      await setDoc(
        doc(firestoreDB, 'courses', courseId as string, 'reviews', user.uid),
        {
          review,
          stars,
          ...
          <form onSubmit={handleReview}>
            <div className="space-y-4">
              <div>
                <label
                  htmlFor="review"
                  className="block text-sm font-medium text-[color:var(--ai-foreground)] mb-1"
                >
