    ...
    async (priceId: string, courseId: string) => {
      ...
      try {
        ...
        toast.showToast({
          ...
          message:
            'You are being redirected to Stripe to complete your purchase.',
          duration: 4000,
        });
  ...
  const getCoursePrice = useCallback(
    (course: any) => {
      if (!course?.priceProduct)
        return { amount: 0, currency: 'RON', priceId: '' };

      const product = products?.find(
        (product: any) => product.id === course.priceProduct.id
      );
      const price = product?.prices.find(
        (price: any) => price.id === course.price
      );
      return {
      ...
      >
        <div className="text-center">
          ...
          <p className="max-w-2xl mx-auto text-lg text-[color:var(--ai-muted)]">
            Master the technologies shaping our future with courses designed by
            AI experts for tomorrow&apos;s innovators.
          </p>
        </div>
      ...
      >
        ...
        {courseLoadingStates && courseLoadingStates['all'] === 'loading' && (
          <div className="col-span-full flex flex-col items-center justify-center py-16">
            ...
            <p className="text-[color:var(--ai-muted)] text-lg">
              Loading courses...
            </p>
          </div>
        )}
        ...
        {courseLoadingStates && courseLoadingStates['all'] === 'error' && (
          <div className="col-span-full flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-[color:var(--ai-muted)] mb-6">
              There was a problem loading the courses. Please try refreshing the
              page.
            </p>
          ...
          Object.keys(courses).map((key: any) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="absolute inset-0 rounded-xl bg-gradient-to-r from-[color:var(--ai-primary)] via-[color:var(--ai-secondary)] to-[color:var(--ai-accent)] opacity-0 group-hover:opacity-100 -z-10 blur transition-opacity duration-300"
                    ...
                    <img
                      src={
                        (course.priceProduct &&
                          products?.find(
                            (product: any) =>
                              product?.id === course.priceProduct?.id
                          )?.images?.[0]) ||
                        null
                      }
                      alt={course.name}
