...
export const useHeroStats = (): HeroStats => {
  ...
  useEffect(() => {
    ...
    const uniqueStudents = new Set(
      userPaidProducts.map((product: any) => product.metadata?.userId || '')
    );
    const totalStudents =
      uniqueStudents.size > 0 ? uniqueStudents.size : userPaidProducts.length;
    ...
    const finalTechnologies =
      topTechnologies.length >= 3 ? topTechnologies : defaultTechnologies;
    const newStats = {
      totalCourses,
