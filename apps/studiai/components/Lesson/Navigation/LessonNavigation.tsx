    ...
    <div className="w-full">
      <div className="bg-gradient-to-r from-[color:var(--ai-primary)]/15 via-[color:var(--ai-secondary)]/15 to-[color:var(--ai-accent)]/10 py-4 px-6 border-b border-[color:var(--ai-card-border)]/50">
        ...
        <h3 className="font-semibold text-[color:var(--ai-foreground)] flex items-center justify-between">
          <div className="flex items-center">
            <FiBookOpen
              className="mr-2 text-[color:var(--ai-primary)]"
              size={18}
            />
      ...
      <div className="p-6 bg-[color:var(--ai-card-bg)]/30 flex flex-col gap-5">
        ...
        {(expanded || lessons.length > 0) && (
          <div
            className={`mt-2 transition-all duration-300 ${expanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}
          >
            {' '}
            ...
            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
              {lessons.map(lesson => (
                <div
                  ...
                  onClick={() =>
                    !lesson.isLocked && onNavigateLesson(lesson.id)
                  }
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    <div
                      className={`p-1 rounded-full ${
                        lesson.isCompleted
                          ...
                          : lesson.isLocked
                            ? 'bg-[color:var(--ai-foreground-muted)]/20'
                    ...
                    <div className="truncate">
                      <p
                        className={`text-sm font-medium truncate ${lesson.isLocked ? 'text-[color:var(--ai-foreground-muted)]' : ''}`}
                      >
