...
export default function ModalComponent({
  ...
  const defaultClassNames = {
    backdrop:
      'z-50 backdrop-blur-md backdrop-saturate-150 bg-white/70 dark:bg-[color:var(--ai-background)]/60 w-screen min-h-[100dvh] fixed inset-0',
    wrapper:
      'z-50 min-h-[100dvh] w-full flex flex-col justify-center items-center overflow-hidden',
    ...
    header:
      'flex flex-row justify-between items-center w-full p-2 border-b border-[color:var(--ai-card-border)] text-[color:var(--ai-foreground)]',
    closeButton:
      'z-50 flex flex-row justify-end items-center rounded-full text-[color:var(--ai-foreground)] hover:bg-[color:var(--ai-card-border)]/40 dark:hover:bg-[color:var(--ai-card-border)]/20 p-2 pr-4 cursor-pointer scale-150',
  };
      ...
      <ModalContent>
        {closeModal => (
          <>
            ...
            <ModalBody className="flex flex-col gap-2 w-full h-full p-0">
              {modalBody}
            </ModalBody>
            {!footerDisabled && (
              <ModalFooter className="w-full flex flex-row justify-between gap-2 z-50">
                <div>{modalBottomComponent}</div>
                {footerButtonText && (
                  <div
