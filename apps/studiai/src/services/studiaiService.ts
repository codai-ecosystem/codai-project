...
export class StudiaiService {
  private data: Map<string, StudiaiData> = new Map();

  async getAll(): Promise<StudiaiData[]> {
    return Array.from(this.data.values());
  }

  async getById(id: string): Promise<StudiaiData | null> {
  ...
  // Business logic methods specific to studiai
  async processBusinessLogic(data: any): Promise<any> {
    // Implement studiai-specific business logic here
