...
describe('Tools API Integration Tests', () => {
  ...
  describe('DELETE /api/tools/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/tools/${testRecord.id}`)
        .expect(204);
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/tools/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/tools/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/tools/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/tools?name=${encodeURIComponent(maliciousInput)}`)
