...
describe('Wallet API Integration Tests', () => {
  ...
  describe('GET /api/wallet/:id', () => {
    ...
    it('should return 404 for non-existent ID', async () => {
      await request(app)
        .get('/api/wallet/non-existent-id')
        .expect(404);
    });

    it('should handle invalid ID format', async () => {
      await request(app)
        .get('/api/wallet/invalid-id-format')
        .expect(400);
    });
  });
  ...
  describe('PUT /api/wallet/:id', () => {
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .put('/api/wallet/non-existent-id')
  ...
  describe('DELETE /api/wallet/:id', () => {
    it('should delete record successfully', async () => {
      ...
      await request(app)
        .delete(`/api/wallet/${testRecord.id}`)
        .expect(204);
    ...
    it('should return 404 for non-existent record', async () => {
      await request(app)
        .delete('/api/wallet/non-existent-id')
        .expect(404);
    });
    ...
    it('should handle cascade deletions', async () => {
      ...
      await request(app)
        .delete(`/api/wallet/${parentRecord.id}`)
        .expect(204);

      // Verify both are deleted
      await request(app)
        .get(`/api/wallet/${parentRecord.id}`)
        .expect(404);

      await request(app)
        .get(`/api/wallet/${childRecord.id}`)
        .expect(404);
    });
  });
  ...
  describe('Security', () => {
    it('should prevent SQL injection attacks', async () => {
      ...
      await request(app)
        .get(`/api/wallet?name=${encodeURIComponent(maliciousInput)}`)
