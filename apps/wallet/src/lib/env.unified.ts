...
// Define logger interface to avoid using 'any'
interface Logger {
...
const clientEnvSchema = z.object({
  // Firebase Configuration (Required for core functionality)
...
class EnvCache {
  ...
  static getInstance(): EnvCache {
    if (EnvCache.instance === undefined) {
  ...
  isValid(): boolean {
    return (
      ...
      Date.now() - this.lastValidation < this.cacheTimeout
    );
  }
...
export function getFirebaseConfig(): {
  ...
  if (
    env.NEXT_PUBLIC_FIREBASE_MEASUREMENT_ID != null &&
  ...
  if (
    env.NEXT_PUBLIC_FIREBASE_DATABASE_URL != null &&
