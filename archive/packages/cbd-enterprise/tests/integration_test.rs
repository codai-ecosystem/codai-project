...
use std::sync::Arc;
use cbd_core::{CBDConfig, StorageConfig, VectorConfig, ClusterConfig, SecurityConfig, PerformanceConfig, StorageEngine, VectorIndex, Transaction, VectorSearchResult, VectorStats};
...
async fn test_cbd_engine_basic_functionality() -> Result<()> {
    ...
    let vector_index: Arc<dyn VectorIndex + Send + Sync> = Arc::new(HNSWVectorIndex::new(&config.vector));
    ...
    let vector_index_ref = &engine.vector_index;
    
    // Initialize vector index
    vector_index_ref.initialize().await?;
