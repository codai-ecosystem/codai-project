...
export function MemoryResults({
  ...
  // Load memories from our fixed API
  useEffect(() => {
    const loadMemories = async () => {
      setIsLoading(true);
      try {
        console.log('Loading memories for results component...');
        const response = await fetch('/api/mcp/read-graph');
        if (response.ok) {
          const data = await response.json();
          console.log('Memory results API response:', data);
  ...
  const filteredAndSortedMemories = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      filtered = memories.filter(
        memory =>
          ...
          memory.entity_type?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    // Apply type filter
    if (filterType !== 'all') {
      filtered = filtered.filter(memory => memory.entity_type === filterType);
    }
  ...
  const handleDeleteMemory = async (memoryId: string) => {
    try {
      // For now, just remove from local state since API deletion might not be available
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const deletedCount = selectedMemories.size;
      setSelectedMemories(new Set());
      toast.success(`${deletedCount} memories deleted`);
    } catch (error) {
