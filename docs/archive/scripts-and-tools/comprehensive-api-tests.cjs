...
async function testHealthEndpoint(serviceName, port) {
    // Use correct health endpoint path for Gateway
    const healthPath = serviceName === 'gateway' ? '/api/gateway/health' : '/health';
    const url = `${TEST_CONFIG.baseUrl}:${port}${healthPath}`;
