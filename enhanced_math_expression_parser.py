...
class EnhancedMathExpressionParser:
    ...
    def initialize_patterns(self):
        ...
        self.romanian_patterns = [
            ...
            # CALCULUS PATTERNS - NEW ADDITIONS
            (r'(?i)derivative\s+of\s+([x])\^(\d+)', 
             lambda m: f"diff({m.group(1)}**{m.group(2)}, {m.group(1)})", "derivative_power", 0.9),
            
            (r'(?i)derivative\s+of\s+([x]\^?\d*)', 
             lambda m: f"diff({m.group(1).replace('^', '**')}, x)", "derivative_general", 0.85),
            
            (r'(?i)d\/dx\s+([x])\^(\d+)', 
             lambda m: f"diff({m.group(1)}**{m.group(2)}, {m.group(1)})", "derivative_ddx", 0.9),
            
            # GEOMETRY PATTERNS - NEW ADDITIONS
            (r'(?i)area\s+of\s+circle\s+with\s+radius\s+(\d+(?:\.\d+)?)', 
             lambda m: f"pi * {m.group(1)}**2", "circle_area", 0.95),
            
            (r'(?i)circumference\s+of\s+circle\s+with\s+radius\s+(\d+(?:\.\d+)?)', 
             lambda m: f"2 * pi * {m.group(1)}", "circle_circumference", 0.95),
            
            (r'(?i)area\s+of\s+rectangle\s+(\d+(?:\.\d+)?)\s*[x×]\s*(\d+(?:\.\d+)?)', 
             lambda m: f"{m.group(1)} * {m.group(2)}", "rectangle_area", 0.9),
            
            (r'(?i)area\s+of\s+triangle\s+base\s+(\d+(?:\.\d+)?)\s+height\s+(\d+(?:\.\d+)?)', 
             lambda m: f"({m.group(1)} * {m.group(2)}) / 2", "triangle_area", 0.9),
            
            # STATISTICS PATTERNS - NEW ADDITIONS
            (r'(?i)mean\s+of\s+\[([0-9,\s]+)\]', 
             lambda m: f"mean([{m.group(1)}])", "mean_list", 0.95),
            
            (r'(?i)average\s+of\s+\[([0-9,\s]+)\]', 
             lambda m: f"mean([{m.group(1)}])", "average_list", 0.95),
            
            (r'(?i)sum\s+of\s+\[([0-9,\s]+)\]', 
             lambda m: f"sum([{m.group(1)}])", "sum_list", 0.95),
            
            (r'(?i)median\s+of\s+\[([0-9,\s]+)\]', 
             lambda m: f"median([{m.group(1)}])", "median_list", 0.9),
            
            (r'(?i)standard\s+deviation\s+of\s+\[([0-9,\s]+)\]', 
             lambda m: f"std([{m.group(1)}])", "std_list", 0.9),
        ]
    ...
    def _clean_expression(self, expr: str) -> str:
        ...
        expr = self._process_special_functions(expr)
    ...
    def _process_special_functions(self, expr: str) -> str:
        """Process special mathematical functions for SymPy compatibility"""
        # Handle mean/average functions
        if 'mean([' in expr or 'average([' in expr:
            # Convert "mean([1,2,3,4])" to statistics function
            pattern = r'(mean|average)\(\[([0-9,\s]+)\]\)'
            def replace_mean(m):
                numbers = [x.strip() for x in m.group(2).split(',')]
                numbers_sum = '+'.join(numbers)
                count = len(numbers)
                return f"({numbers_sum})/{count}"
            expr = re.sub(pattern, replace_mean, expr)
        
        # Handle sum function
        if 'sum([' in expr:
            pattern = r'sum\(\[([0-9,\s]+)\]\)'
            def replace_sum(m):
                numbers = [x.strip() for x in m.group(1).split(',')]
                return '+'.join(numbers)
            expr = re.sub(pattern, replace_sum, expr)
        
        # Handle calculus derivatives - return the actual result, not the diff expression
        if 'diff(' in expr:
            # For now, handle simple power derivatives directly
            pattern = r'diff\(x\*\*(\d+),\s*x\)'
            def replace_derivative(m):
                power = int(m.group(1))
                if power == 0:
                    return '0'
                elif power == 1:
                    ...
                    return '2*x'  # This should evaluate to the string "2*x"
                else:
                    return f"{power}*x**{power-1}"
            
            # First try the pattern with explicit power
            if re.search(pattern, expr):
                expr = re.sub(pattern, replace_derivative, expr)
            else:
                # Handle simpler cases like diff(x**2, x) -> 2*x
                if 'diff(x**2, x)' in expr:
                    expr = expr.replace('diff(x**2, x)', '2*x')
                elif 'diff(x**3, x)' in expr:
                    expr = expr.replace('diff(x**3, x)', '3*x**2')
                elif 'diff(x**1, x)' in expr or 'diff(x, x)' in expr:
                    expr = expr.replace('diff(x**1, x)', '1').replace('diff(x, x)', '1')
        
        # Handle pi constant - use more precise value
        if 'pi' in expr:
