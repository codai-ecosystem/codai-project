...
export interface SearchScore {
    contentRelevance: number;      // Direct content matching
    semanticSimilarity: number;    // Vector similarity
    temporalRelevance: number;     // Recency boost
    relationshipBoost: number;     // Connections to other relevant memories
    importanceWeight: number;      // Memory importance factor
    contextualRelevance: number;   // Session/project context
...
export interface SearchContext {
    ...
    recentMemories?: string[];     // Recently accessed memory IDs
...
export interface AdvancedSearchOptions {
    ...
    temporalWeight?: number;       // 0-1, how much to weight recent memories
...
export interface EnhancedMemory {
    ...
    relationshipContext?: string;  // Why this memory is relevant through relationships
...
export class AdvancedSearchEngine {
    ...
    private searchPatterns = new Map<string, number>();
     ...
     */
    async expandQuery(originalQuery: string, context: SearchContext): Promise<string[]> {
    ...
    async calculateAdvancedRelevance(
        ...
        for (const memory of memories) {
            const searchScore = await this.calculateSearchScore(
                memory,
                originalQuery,
                searchTerms,
                context,
     ...
     */
    private async clusterResults(memories: EnhancedMemory[]): Promise<MemoryCluster[]> {
        ...
        for (let i = 0; i < memories.length; i++) {
            ...
            if (!memory || clustered.has(memory.id)) continue;
            ...
            for (let j = i + 1; j < memories.length; j++) {
                ...
                if (!otherMemory || clustered.has(otherMemory.id)) continue;
    ...
    private matchesAnyTerm(content: string, terms: string[]): boolean {
        const lowerContent = content.toLowerCase();
        return terms.some(term => lowerContent.includes(term.toLowerCase()));
    }

    private matchesMetadata(metadata: any, terms: string[]): boolean {
        const metadataText = JSON.stringify(metadata).toLowerCase();
        return terms.some(term => metadataText.includes(term.toLowerCase()));
    }

    private matchesRelationships(memory: any, terms: string[]): boolean {
    ...
    private async calculateSemanticRelevance(query: string, memory: any): Promise<number> {
        if (!this.openai || !memory.embedding) return 0;
    ...
    private calculateTemporalRelevance(memory: any, temporalWeight: number): number {
    ...
    private calculateContextualRelevance(memory: any, context: SearchContext): number {
        let score = 0;
    ...
    private calculateRelationshipBoost(memory: any, context: SearchContext): number {
        // Placeholder - would calculate based on actual relationships
        return 0;
    }

    private calculateFinalScore(scores: SearchScore): number {
    ...
    private async calculateContentSimilarity(memory1: EnhancedMemory, memory2: EnhancedMemory): Promise<number> {
    ...
    private findRelatedMemories(memory: EnhancedMemory, allMemories: any[]): any[] {
    ...
    private calculateAverageRelevance(memories: EnhancedMemory[]): number {
    ...
    private extractTimeReferences(query: string): TimeReference[] {
        ...
        const timePatterns = [
            ...
            { pattern: /yesterday/i, type: 'relative' as const },
    ...
    private determineSearchStrategy(analysis: QueryAnalysis): string {
        if (analysis.expandedTerms.length > 1) return 'Query expansion with semantic search';
        if (analysis.entities.length > 0) return 'Entity-focused search';
        if (analysis.timeReferences.length > 0) return 'Temporal-aware search';
    ...
    private findHistoricalSuggestions(partialQuery: string): string[] {
    ...
    private findRelatedTermsFromHistory(query: string, searchHistory: string[]): string[] {
