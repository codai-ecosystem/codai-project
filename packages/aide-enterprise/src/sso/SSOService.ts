...
export class EnterpriseSSO {
  ...
  private azureClient?: ConfidentialClientApplication
  ...
  private async initializeProviders(): Promise<void> {
    switch (this.config.provider) {
      ...
      case 'azure-ad':
        this.azureClient = new ConfidentialClientApplication({
          auth: {
  ...
  async getLoginUrl(): Promise<string> {
    switch (this.config.provider) {
      ...
      case 'azure-ad':
        ...
        return await this.azureClient.getAuthCodeUrl(authCodeUrlParameters)
  ...
  private async handleAzureCallback(code: string, state?: string): Promise<SSOSession> {
    ...
    const user = await this.createUser({
      id: response.uniqueId || response.account?.localAccountId || '',
    ...
    return this.createSession(user, response.accessToken, response.refreshToken || undefined)
  }
  ...
  private async refreshAzureToken(refreshToken: string): Promise<SSOSession> {
    ...
    const user = await this.getUserById(response.uniqueId || response.account?.localAccountId || '')
    if (!user) throw new Error('User not found')
