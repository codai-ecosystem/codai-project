...
class CBDMemoryStore {
    ...
    async initializeCollection() {
        try {
            ...
            if (response.ok) {
                console.log('✅ MemorAI collection initialized in CBD Database');
            }
        } catch (error) {
            console.log('ℹ️  MemorAI collection may already exist in CBD Database');
        }
    }
    ...
    async store(agentId, content, metadata = {}) {
        try {
            ...
            const result = await response.json();
            console.log(`💾 Stored memory: ${memory.id} for agent: ${agentId}`);
            return { success: true, memory: result.document || memory };
        } catch (error) {
            console.error('❌ CBD store error:', error);
            return { success: false, error: error.message };
        }
    ...
    async search(agentId, query, options = {}) {
        try {
            ...
            const response = await fetch(`${this.baseUrl}/document/${this.collectionName}/search`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(searchParams)
            });

            if (!response.ok) throw new Error(`CBD search failed: ${response.statusText}`);
            
            const result = await response.json();
            return { success: true, memories: result.documents || [] };
        } catch (error) {
            console.error('❌ CBD search error:', error);
    ...
    async getAll(agentId, limit = 100) {
        try {
            const response = await fetch(`${this.baseUrl}/document/${this.collectionName}`, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            ...
            return { success: true, memories: filteredMemories };
        } catch (error) {
            console.error('❌ CBD getAll error:', error);
            return { success: false, error: error.message, memories: [] };
        }
...
app.post('/', async (req, res) => {
    ...
    if (method === 'tools/call') {
        ...
        try {
            switch (name) {
                case 'remember':
                    // Store in CBD Database instead of memory
                    ...
                    if (!storeResult.success) {
                        res.status(500).json({
                            ...
                            error: {
                                code: -32603,
                                message: `Failed to store memory: ${storeResult.error}`
                            }
                        });
                        break;
                    }
