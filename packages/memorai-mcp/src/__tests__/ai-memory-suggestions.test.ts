...
describe('AiMemorySuggestionEngine', () => {
  ...
  describe('Event System', () => {
    it('should emit events for suggestion generation', async () => {
      // Store additional memories that will generate suggestions with more related content
      await memoryStore.store('ai-suggestion-agent', 'Working on authentication system with JWT tokens', {
        entityType: 'development_task',
        importance: 9,
        tags: ['development', 'authentication', 'security', 'jwt']
      });

      await memoryStore.store('ai-suggestion-agent', 'Implementing user login functionality with password hashing', {
        entityType: 'development_task',
        importance: 8,
        tags: ['development', 'login', 'user-management', 'security']
      });

      await memoryStore.store('ai-suggestion-agent', 'Setting up OAuth2 integration for social login', {
        entityType: 'development_task',
        importance: 8,
        tags: ['development', 'oauth', 'authentication', 'social-login']
      });

      // Create a lower minimum relevance threshold to ensure suggestions are generated
      const testEngine = new AiMemorySuggestionEngine(memoryStore, {
        enabled: true,
        maxSuggestions: 5,
        minRelevanceScore: 0.1, // Lower threshold to ensure suggestions
        enableCaching: true,
        cacheTTL: 5 * 60 * 1000,
      ...
      testEngine.on('suggestions_generated', (eventData) => {
        eventReceived = true;
        suggestionsCount = eventData.suggestionCount;
      });

      const context: SuggestionContext = {
        agentId: 'ai-suggestion-agent',
        currentActivity: 'development',
        activeProjects: ['authentication-project'],
        timeContext: 'current_sprint'
      };

      const suggestions = await testEngine.generateSuggestions(context);
