export class DataSynchronizer {
    ...
    async applyOperation(operation) {
        ...
        try {
            ...
            return { success: false, conflicts: [{ error: error.message, operation }] };
        }
    }
    // Create operation
    async handleCreate(id, data, version, timestamp, author) {
    ...
    // Update operation
    async handleUpdate(id, data, version, timestamp, author) {
        if (!this.localData.has(id)) {
    ...
    // Delete operation
    async handleDelete(id, version, timestamp, author) {
        if (!this.localData.has(id)) {
            throw new Error(`Data with id ${id} does not exist`);
        }
    ...
    async handlePatch(id, data, path, version, timestamp, author) {
        if (!this.localData.has(id)) {
            throw new Error(`Data with id ${id} does not exist`);
        }
        const existing = this.localData.get(id);
        ...
        switch (this.conflictResolution.strategy) {
            ...
            case 'custom':
                ...
                if (this.conflictResolution.resolver) {
                    const resolved = this.conflictResolution.resolver(this.localData.get(operation.id), operation.data);
        ...
        try {
            ...
            if (typeof existing === 'object' && typeof operation.data === 'object') {
                const merged = {
                    ...
                    _metadata: {
                        ...existing._metadata,
                        version: Math.max(((_a = existing._metadata) === null || _a === void 0 ? void 0 : _a.version) || 0, operation.version),
                        timestamp: Date.now(),
                        author: 'merged',
                        mergedFrom: [(_b = existing._metadata) === null || _b === void 0 ? void 0 : _b.author, operation.author],
                    },
                };
    ...
    async resolveConflict(conflictId, resolution) {
        ...
        return this.handleUpdate(conflict.id, resolution, conflict.version + 1, Date.now(), 'user-resolved');
    }
    // Create a sync operation
    createSyncOperation(id, type, data, author, path) {
