...
async function addAsSubmodule(service) {
  ...
  try {
    ...
    if (fs.existsSync(servicePath)) {
      console.log(
        chalk.yellow(`⚠️  ${service.name} already exists in services/`)
      );
    ...
    if (!exists) {
      console.log(
        chalk.red(
          `❌ Repository ${service.repo} does not exist or is not accessible`
        )
      );
    ...
    console.log(
      chalk.green(`✅ Successfully added ${service.name} to services/`)
    );
    return { status: 'added', service };
  } catch (error) {
    console.log(
      chalk.red(`❌ Failed to add ${service.name}: ${error.message}`)
    );
...
async function migrateFromApps(service) {
  ...
  try {
    ...
    if (fs.existsSync(servicePath)) {
      console.log(
        chalk.yellow(
          `⚠️  ${service.name} already in services/, skipping migration`
        )
      );
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      if (packageJson.name && packageJson.name.includes('template')) {
        console.log(
          chalk.blue(
            `📋 ${service.name} in apps/ is a template, will replace with repository`
          )
        );
      }
    }
    ...
    console.log(
      chalk.red(`❌ Failed to migrate ${service.name}: ${error.message}`)
    );
    return { status: 'error', service, error: error.message };
  }
}
...
async function main() {
  ...
  console.log(
    chalk.cyan(
      'Adding all available repositories from codai-ecosystem organization...\n'
    )
  );
  ...
  if (results.added.length > 0) {
    ...
    try {
      ...
      console.log(
        chalk.red(`❌ Failed to update submodules: ${error.message}`)
      );
    }
  }
  ...
  results.added.forEach(service =>
    console.log(chalk.green(`   - ${service.name} (${service.description})`))
  );

  console.log(
    chalk.yellow(`⚠️  Existing: ${results.exists.length} repositories`)
  );
  results.exists.forEach(service =>
    console.log(chalk.yellow(`   - ${service.name} (${service.description})`))
  );

  console.log(
    chalk.red(`❌ Not Found: ${results.not_found.length} repositories`)
  );
  results.not_found.forEach(service =>
    console.log(chalk.red(`   - ${service.name} (${service.description})`))
  );
  ...
  if (results.added.length > 0) {
    ...
    console.log(
      chalk.blue('   3. Check individual services in services/ directory')
    );
  }
}
