...
// Simple fs-extra alternative functions
...
const copy = async (src, dest, options = {}) => {
  ...
  if (stats.isDirectory()) {
    ...
    for (const file of files) {
      ...
      if (options.filter && !options.filter(srcFile, destFile)) continue;
      await copy(srcFile, destFile, options);
    }
  } else {
...
class MetuTemplateApplicator {
  ...
  async applyTemplateToApp(app) {
    ...
    if (fs.existsSync(appPath) && fs.readdirSync(appPath).length > 0) {
      ...
      await copy(appPath, backupPath);
    }

    // Copy template structure
    console.log(`📁 Copying template structure to ${app.name}...`);
    await copy(templateWebPath, appPath, {
      overwrite: true,
      filter: (src, dest) => {
