...
function checkRepoExists(repoName) {
  try {
    execSync(
      `git ls-remote --heads https://github.com/codai-ecosystem/${repoName}.git`,
      {
        stdio: 'pipe',
        timeout: 5000,
      }
...
\turl = https://github.com/codai-ecosystem/${repo.name}.git\n`;
  ...
  try {
    ...
    console.log(
      chalk.red(
        `❌ Failed to add ${repo.name} to .gitmodules: ${error.message}`
      )
    );
    return false;
  }
...
function cloneRepo(repo) {
  ...
  try {
    ...
    execSync(
      `git clone https://github.com/codai-ecosystem/${repo.name}.git ${repoPath}`,
      {
        stdio: 'inherit',
        cwd: process.cwd(),
      }
    ...
    if (repo.hasContent) {
      console.log(
        chalk.green(`✅ Successfully cloned ${repo.name} (with content)`)
      );
    } else {
...
async function main() {
  ...
  console.log(
    chalk.cyan(
      'Integrating all available repositories from codai-ecosystem organization...\n'
    )
  );
  ...
  for (const repo of [...results.cloned, ...results.empty]) {
    try {
      ...
      console.log(
        chalk.green(`✅ Prepared ${repo.name} for submodule conversion`)
      );
    } catch (error) {
      console.log(
        chalk.red(`❌ Failed to prepare ${repo.name}: ${error.message}`)
      );
    }
  }
  ...
  console.log(
    chalk.green(
      `✅ Successfully Integrated: ${results.existing.length + results.cloned.length + results.empty.length}`
    )
  );
  console.log(chalk.blue('   📚 Repositories with Content:'));
  [...results.existing.filter(r => r.hasContent), ...results.cloned].forEach(
    repo =>
      console.log(chalk.green(`      - ${repo.name} (${repo.description})`))
  );

  console.log(chalk.yellow('   📋 Empty Repositories (templates ready):'));
  [...results.existing.filter(r => !r.hasContent), ...results.empty].forEach(
    repo =>
  ...
  if (results.failed.length > 0) {
    ...
    results.failed.forEach(repo =>
      console.log(chalk.red(`   - ${repo.name} (${repo.description})`))
    );
  }

  if (results.notFound.length > 0) {
    console.log(chalk.red(`🔍 Not Found: ${results.notFound.length}`));
    results.notFound.forEach(repo =>
      console.log(chalk.red(`   - ${repo.name} (${repo.description})`))
    );
  }

  const totalIntegrated =
    results.existing.length + results.cloned.length + results.empty.length;
  console.log(
    chalk.bold.cyan(
  ...
  console.log(
    chalk.blue('   3. Empty repositories are ready for content creation')
  );
  console.log(
    chalk.blue('   4. Use individual VS Code windows for service development')
  );
}
