...
class TestInfrastructureFixer {
  ...
  async fixService(service) {
    ...
    console.log(
      `   ${status} ${service.name} (${serviceResult.fixes.length} fixes applied)\n`
    );
  }
...
describe('${service.name} Components', () => {
  ...
  async installGlobalTestDependencies() {
    ...
    try {
      ...
      console.log(
        `⚠️  Failed to install global dependencies: ${error.message}\n`
      );
    }
  }
  ...
  async generateReport() {
    ...
    const successRate =
      this.results.total > 0
        ? Math.round((this.results.fixed / this.results.total) * 100)
        : 0;
...
if (
  process.argv[1] &&
  (process.argv[1].endsWith('fix-test-infrastructure.js') ||
