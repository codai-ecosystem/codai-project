...
console.log('🔧 Fixing test setup files across all services...');
...
for (const service of services) {
    ...
    try {
        if (fs.existsSync(testSetupPath)) {
            ...
            console.log(`  ✅ Fixed test-setup.ts for ${service}`);
        }
    } catch (error) {
        console.error(`  ❌ Error fixing test setup for ${service}:`, error.message);
    }
}

console.log('✅ Test setup fixes completed!');
