...
console.log('🔧 Creating missing UI components for services...');
...
for (const service of services) {
    ...
    try {
        // Create lib directory and utils if it doesn't exist
        ...
        if (!fs.existsSync(utilsPath)) {
            fs.writeFileSync(utilsPath, components['utils.ts']);
            console.log(`  ✅ Created lib/utils.ts for ${service}`);
        }
        ...
        for (const [filename, content] of Object.entries(components)) {
            if (filename === 'utils.ts') continue; // Already handled above

            const componentPath = path.join(uiPath, filename);
            if (!fs.existsSync(componentPath)) {
                fs.writeFileSync(componentPath, content);
                console.log(`  ✅ Created ${filename} for ${service}`);
            }
        }

        // Update the index.ts to export all components
        ...
        fs.writeFileSync(indexPath, validExports.join('\n') + '\n');
        console.log(`  ✅ Updated index.ts for ${service}`);
...
console.log('✅ UI components creation completed!');
