...
async function checkRepositoryExists(repoName) {
  try {
    const { stdout } = await execAsync(
      `git ls-remote https://github.com/codai-ecosystem/${repoName}.git HEAD`
    );
...
async function checkRepositoryEmpty(repoName) {
  try {
    const { stdout } = await execAsync(
      `git ls-remote https://github.com/codai-ecosystem/${repoName}.git | wc -l`
    );
...
async function getRepositoryStats(repoName) {
  ...
  const hasScaffolding = CORE_SERVICES.includes(repoName)
    ? hasLocalScaffolding(repoName)
    : false;
...
async function auditRepositories() {
  ...
  for (const repoName of ECOSYSTEM_REPOSITORIES) {
    ...
    if (!stats.empty) {
      ...
      if (stats.hasScaffolding) {
        ...
        console.log(
          chalk.yellow(
            `  📦 ${repoName} - Empty repo, has local scaffolding, needs push`
          )
        );
      } else {
  ...
  if (categories.contentRich.length > 0) {
    ...
    categories.contentRich.forEach(repo =>
  ...
  if (categories.needsPush.length > 0) {
    ...
    categories.needsPush.forEach(repo =>
  ...
  if (categories.nonExistent.length > 0) {
    ...
    categories.nonExistent.forEach(repo =>
  ...
  if (categories.needsPush.length > 0) {
    console.log(
      chalk.yellow(
        `1. Run "npm run push-scaffolded" to push ${categories.needsPush.length} scaffolded services`
      )
    );
  }
  ...
  if (categories.empty.length > 0) {
    console.log(
      chalk.red(
        `2. Create scaffolding for ${categories.empty.length} empty repositories`
      )
    );
  }
  ...
  if (categories.nonExistent.length > 0) {
    console.log(
      chalk.gray(
        `3. Create ${categories.nonExistent.length} missing repositories in GitHub`
      )
    );
  }
