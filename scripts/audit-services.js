...
function checkServiceFiles(serviceName) {
  ...
  const hasNext = fs.existsSync(path.join(servicePath, 'next.config.js')) || 
                  fs.existsSync(path.join(servicePath, 'next.config.ts'));
  const hasTurbo = fs.existsSync(path.join(servicePath, 'turbo.json'));
  const hasPnpm = fs.existsSync(path.join(servicePath, 'pnpm-workspace.yaml')) ||
...
console.log('\n📊 SUMMARY:');
console.log('===========');
console.log('Audit complete! Review the results above to identify any missing files.');
