...
async function integrateService(service) {
  ...
  if (!repoExists) {
    ...
    if (service.sourceRepo) {
      ...
      console.log(
        `   TODO: Copy from ${service.sourceRepo} to ${service.targetRepo}`
      );
    } else {
  ...
  try {
    ...
    try {
      ...
      execSync(`git clone ${repoUrl} apps/${service.name}`, {
        stdio: 'inherit',
      });
      ...
      if (!hasContent) {
        console.log(
          `⚠️  Repository ${service.name} is empty, creating basic structure...`
        );
      ...
      if (process.platform === 'win32') {
        execSync(`rmdir /s /q "apps\\${service.name}\\.git"`, {
          stdio: 'ignore',
        });
      } else {
      ...
      console.error(
        `❌ Failed to integrate ${service.name}: ${cloneError.message}`
      );
      return {
        success: false,
...
async function generateIntegrationReport(results) {
  ...
  if (successful.length > 0) {
    ...
    successful.forEach(r =>
      console.log(`   - ${r.service.name} (${r.service.domain})`)
    );
  }
  ...
  if (failed.filter(r => r.reason !== 'repository_not_exists').length > 0) {
    ...
    failed
      .filter(r => r.reason !== 'repository_not_exists')
      .forEach(r => {
...
async function main() {
  ...
  for (let priority = 1; priority <= 4; priority++) {
    const priorityServices = CODAI_SERVICES.filter(
