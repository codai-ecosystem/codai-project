...
class BuildOrchestrator {
  ...
  async run() {
    ...
    console.log(
      `🎯 Production Mode: ${BUILD_CONFIG.production ? 'Yes' : 'No'}\n`
    );
  ...
  async createBuildOrder(services) {
    ...
    buildOrder.forEach((service, index) => {
      console.log(
        `   ${index + 1}. ${service.name} (P${service.priority}) - ${service.framework}`
      );
    });
    console.log('');
  ...
  async buildService(service) {
    ...
    try {
      ...
      if (!service.hasBuildScript) {
        console.log(
          `   ⚠️  No build script found for ${service.name}, creating basic build...`
        );
    ...
    const sizeInfo =
      serviceResult.size > 0
        ? ` (${this.formatBytes(serviceResult.size)})`
        : '';

    console.log(
      `   ${status} ${service.name} (${serviceResult.duration}ms)${sizeInfo}\n`
    );
  }
  ...
  async postBuildValidation() {
    ...
    if (failedCritical.length > 0) {
      console.log(
        `⚠️  Critical services failed to build: ${failedCritical.join(', ')}`
      );
    }
  ...
  async generateReport() {
    ...
    const totalSize = Object.values(this.results.services).reduce(
      (sum, service) => sum + service.size,
      0
    );
    ...
    Object.entries(this.results.services).forEach(([name, result]) => {
      ...
      console.log(
        `   ${status} ${name} (P${result.priority}) - ${result.duration}ms${size}`
      );
    ...
    Object.entries(frameworks).forEach(([framework, stats]) => {
      ...
      console.log(
        `   ${framework}: ${stats.successful}/${stats.total} (${rate}%)`
      );
    });
    ...
    console.log(
      `   • Optimize build sizes (current: ${this.formatBytes(totalSize)})`
    );
...
if (
  process.argv[1] &&
  (process.argv[1].endsWith('build-orchestrator.js') ||
