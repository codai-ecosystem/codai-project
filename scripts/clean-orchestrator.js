...
class CleanOrchestrator {
  ...
  async run() {
    ...
    console.log(`🎯 Clean Level: ${this.cleanAll ? 'DEEP' : this.cleanDeps ? 'DEPENDENCIES' : 'STANDARD'}`);
  ...
  async cleanService(service) {
    ...
    try {
      ...
      const buildCleaned = await this.cleanTargets(service.path, CLEANUP_TARGETS.build, 'build artifacts');
      serviceResult.spaceSaved += buildCleaned.spaceSaved;
      serviceResult.cleanedItems.push(...buildCleaned.items);

      // Clean cache
      const cacheCleaned = await this.cleanTargets(service.path, CLEANUP_TARGETS.cache, 'cache files');
      serviceResult.spaceSaved += cacheCleaned.spaceSaved;
      serviceResult.cleanedItems.push(...cacheCleaned.items);

      // Clean temp files
      const tempCleaned = await this.cleanTargets(service.path, CLEANUP_TARGETS.temp, 'temporary files');
      ...
      if (this.cleanDeps || this.cleanAll) {
        const depsCleaned = await this.cleanTargets(service.path, CLEANUP_TARGETS.deps, 'dependencies');
    ...
    const space = serviceResult.spaceSaved > 0 ? ` (${this.formatBytes(serviceResult.spaceSaved)} saved)` : '';
  ...
  async cleanRootWorkspace() {
    ...
    const cleaned = await this.cleanTargets(rootDir, rootTargets, 'root workspace');
    this.results.spaceSaved += cleaned.spaceSaved;

    console.log(`✅ Root workspace cleaned (${this.formatBytes(cleaned.spaceSaved)} saved)\n`);
  }
  ...
  async generateReport() {
    ...
    console.log(`   Total Space Saved: ${this.formatBytes(this.results.spaceSaved)} 💾`);
    ...
    Object.entries(this.results.services).forEach(([name, result]) => {
      ...
      const space = result.spaceSaved > 0 ? ` (${this.formatBytes(result.spaceSaved)})` : '';
    ...
    if (this.results.spaceSaved > 0) {
      console.log(`   • Freed up ${this.formatBytes(this.results.spaceSaved)} of disk space`);
    }
    if (!this.cleanDeps && !this.cleanAll) {
...
if (process.argv[1] && (process.argv[1].endsWith('clean-orchestrator.js') || import.meta.url === `file://${process.argv[1]}`)) {
