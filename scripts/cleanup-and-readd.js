...
const EMPTY_SERVICES = ['explorer', 'marketai', 'analizai', 'tools'];
...
async function main() {
  ...
  console.log(
    chalk.cyan(
      'Cleaning up problematic directories and re-adding repositories...\n'
    )
  );
    ...
    if (fs.existsSync(gitmodulesPath)) {
      ...
      for (const service of EMPTY_SERVICES) {
        const pattern = new RegExp(
          `\\[submodule "services/${service}"\\][\\s\\S]*?(?=\\[submodule|$)`,
          'g'
        );
  ...
  console.log(
    chalk.green(`✅ Successfully added: ${successCount} repositories`)
  );
  console.log(chalk.red(`❌ Failed to add: ${errorCount} repositories`));
  console.log(
    chalk.yellow(`🗑️  Removed empty: ${EMPTY_SERVICES.length} repositories`)
  );

  console.log(chalk.bold.cyan('\n📊 Final Status:'));
  console.log(chalk.blue('   Run "npm run status" to verify the integration'));
  console.log(
    chalk.blue('   All core services should now be properly integrated')
  );
}
