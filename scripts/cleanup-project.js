        ...
        for (const entry of entries) {
          ...
          if (
            ...
            if (
              ...
              entry.name.includes('lint-backup') ||
              entry.name.includes('prod-backup') ||
              entry.name.includes('console-backup')
            ) {
              files.push(fullPath);
            }
          }
    ...
    if (files.length > 0) {
      ...
      for (const file of files) {
        ...
        if (removed) {
          ...
          console.log(
            `❌ Failed to remove: ${path.relative(this.rootDir, file)}`
          );
        }
      }
    } else {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (
            ...
            entry.isFile() &&
            (entry.name.endsWith('.log') || entry.name.endsWith('.err'))
          ) {
            files.push(fullPath);
          }
        }
  ...
  async auditPackages() {
    ...
    for (const pkgFile of packageFiles) {
      ...
      if (analysis) {
        ...
        console.log(
          `📦 ${analysis.name}: ${analysis.dependencies.length} deps, ${analysis.devDependencies.length} devDeps`
        );
      }
    }
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (
            ...
            entry.isFile() &&
            /\.(ts|tsx|js|jsx|md)$/.test(entry.name)
          ) {
            try {
...
if (
  ...
  process.argv[1].endsWith('cleanup-project.js')
) {
  main();
}
