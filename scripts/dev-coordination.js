...
Development Modes:
${Object.entries(DEVELOPMENT_MODES).map(([mode, config]) => 
...
`);
  ...
  if (sequential) {
    ...
    // Execute in parallel
    const promises = services.map(service => executeServiceAction(service, action));
...
function executeServiceAction(service, action) {
  return new Promise((resolve, reject) => {
    ...
    process.on('close', (code) => {
      ...
      } else if (action !== 'open') { // VS Code opening might return non-zero but still work
...
function showServiceQuickStatus(service) {
  ...
  console.log(`📊 ${service}: ${info} | VS Code: ${hasVSCode ? '✅' : '❌'} | Agent: ${hasAgent ? '✅' : '❌'}`);
}
...
if (action === 'open') {
  ...
  executeForServices(config.services, action, action === 'build' || action === 'test');
}
