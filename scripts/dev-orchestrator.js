  ...
  for (const serviceName of serviceNames) {
    ...
    const process = startService(serviceName, {
      ...
      port: options.basePort
        ? options.basePort + processes.length
        : service.port,
    });
...
program
  ...
  .option(
    '--port <port>',
    'Base port number (auto-increment for multiple services)'
  )
  ...
  .action(options => {
    ...
    if (options.all) {
      ...
      servicesToStart = availableServices.filter(
        name => SERVICES[name].priority === priority
      );
    ...
    logStatus(
      `Starting ${servicesToStart.length} services: ${servicesToStart.join(', ')}`,
      'info'
    );
  ...
  .action(options => {
    ...
    Object.entries(SERVICES).forEach(([name, config]) => {
      ...
      if (!options.available || available) {
        console.log(
          `${status} ${chalk.bold(name)} (${priority}) - ${config.description}`
        );
...
program
  ...
  .action(() => {
    ...
    console.log(chalk.blue('\n📊 Ecosystem Status:\n'));
    console.log(
      `Services Available: ${chalk.green(availableCount)}/${totalServices}`
    );
    console.log(
      `Completion: ${chalk.yellow(Math.round((availableCount / totalServices) * 100))}%`
    );

    const priorityStats = {};
    Object.values(SERVICES).forEach(service => {
      priorityStats[service.priority] =
        (priorityStats[service.priority] || 0) + 1;
    });

    console.log('\nPriority Distribution:');
    Object.entries(priorityStats).forEach(([priority, count]) => {
      const available = availableServices.filter(
        name => SERVICES[name].priority === parseInt(priority)
      ).length;
...
if (
  ...
  process.argv[1].endsWith('dev-orchestrator.js')
) {
  program.parse();
}
