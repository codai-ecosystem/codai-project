...
function main() {
  ...
  Object.entries(SERVICES).forEach(([name, config]) => {
    ...
    console.log(
      `  ${icon} ${name.padEnd(12)} (P${config.priority}) - ${config.description}`
    );
  ...
  console.log(
    `Missing: ${totalServices - servicesInServices - servicesInApps}`
  );

  console.log('\n🛠️ Available Commands:');
  console.log(
    '  npm run setup       - Initialize/migrate to v2.0 architecture'
  );
  ...
  console.log(
    '🚀 Codai Ecosystem - Building the future of AI-native development'
  );
  console.log(
    '================================================================================\n'
  );
...
if (
  ...
  process.argv[1].endsWith('ecosystem-status-simple.js')
) {
  main();
}
