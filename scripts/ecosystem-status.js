...
const SERVICES = {
    codai: { port: 3000, priority: 1, description: 'Central Platform & AIDE Hub', repo: 'codai-ecosystem/codai' },
    memorai: { port: 3001, priority: 1, description: 'AI Memory & Database Core', repo: 'codai-ecosystem/memorai' },
    logai: { port: 3002, priority: 1, description: 'Identity & Authentication Hub', repo: 'codai-ecosystem/logai' },
    bancai: { port: 3003, priority: 2, description: 'Financial Platform', repo: 'codai-ecosystem/bancai' },
    wallet: { port: 3004, priority: 2, description: 'Programmable Wallet', repo: 'codai-ecosystem/wallet' },
    fabricai: { port: 3005, priority: 2, description: 'AI Services Platform', repo: 'codai-ecosystem/fabricai' },
    studiai: { port: 3006, priority: 3, description: 'AI Education Platform', repo: 'codai-ecosystem/studiai' },
    sociai: { port: 3007, priority: 3, description: 'AI Social Platform', repo: 'codai-ecosystem/sociai' },
    cumparai: { port: 3008, priority: 3, description: 'AI Shopping Platform', repo: 'codai-ecosystem/cumparai' },
    publicai: { port: 3009, priority: 4, description: 'Civic AI & Transparency Tools', repo: 'codai-ecosystem/publicai' },
    x: { port: 3010, priority: 4, description: 'AI Trading Platform', repo: 'codai-ecosystem/x' }
};
...
function logHeader(text, char = '=') {
    ...
    console.log(chalk.blue(char + ' '.repeat(leftPad) + text + ' '.repeat(rightPad) + char));
    console.log(chalk.blue(char.repeat(width)));
}
...
function getServiceStatus(serviceName) {
    ...
    if (existsSync(servicePath)) {
        ...
        if (hasPackageJson) {
            try {
                const packageContent = readFileSync(join(servicePath, 'package.json'), 'utf8');
        ...
        if (hasPackageJson) {
            try {
                const packageContent = readFileSync(join(appPath, 'package.json'), 'utf8');
                packageInfo = JSON.parse(packageContent);
            } catch (error) {
                // Ignore parse errors
            }
    ...
    return {
        ...
        isGitRepo,
        packageInfo,
        path: location === 'services' ? servicePath : (location === 'apps' ? appPath : null)
    };
}
...
function getGitStatus() {
    try {
        ...
        const branch = execSync('git branch --show-current', { encoding: 'utf8', cwd: PROJECT_ROOT }).trim();
...
function getSubmoduleStatus() {
    try {
        const submodules = execSync('git submodule status', { encoding: 'utf8', cwd: PROJECT_ROOT });
        return submodules.split('\n').filter(line => line.trim()).map(line => {
...
function main() {
    ...
    console.log(`Changes: ${gitStatus.hasChanges ? chalk.yellow(`${gitStatus.changeCount} files`) : chalk.green('Clean')}`);
    ...
    // Priority groups
    Object.entries(priorityGroups).sort().forEach(([priority, services]) => {
        ...
        services.forEach(({ name, config, status }) => {
            ...
            console.log(`    ${locationIcon} ${name.padEnd(12)} - ${config.description}`);
            console.log(`      Location: ${status.location.padEnd(8)} Package: ${packageIcon} Git: ${gitIcon}`);

            if (status.packageInfo) {
                console.log(`      Version: ${status.packageInfo.version}, Framework: ${status.packageInfo.dependencies?.next ? 'Next.js' : 'Unknown'}`);
            }
        });
    });
    ...
    console.log(`Available: ${chalk.green(availableServices)}/${totalServices} (${Math.round(availableServices / totalServices * 100)}%)`);
    ...
    if (isHybrid) {
        console.log(`${chalk.yellow('⚠️')} Hybrid Architecture (Migration in Progress)`);
        console.log('   Run migration script to complete transition to v2.0');
    } else if (isV2Architecture) {
        console.log(`${chalk.green('✅')} V2.0 Architecture (Services + Submodules)`);
    } else if (isV1Architecture) {
        console.log(`${chalk.blue('ℹ️')} V1.0 Architecture (Apps + Subtrees)`);
        console.log('   Consider upgrading to v2.0 for better development workflow');
    } else {
