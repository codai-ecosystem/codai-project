...
async function checkSourceRepo(sourceRepo) {
  try {
    execSync(`git ls-remote https://github.com/${sourceRepo}.git`, {
      stdio: 'ignore',
    });
    return true;
  } catch (error) {
...
async function migrateSource(migration) {
  ...
  try {
    ...
    execSync(
      `git clone https://github.com/${migration.sourceRepo}.git ${tempDir}`,
      { stdio: 'inherit' }
    );
  ...
  try {
    ...
    try {
      ...
      const sourceExists = await fs
        .access(sourcePackageJson)
        .then(() => true)
        .catch(() => false);

      if (sourceExists) {
        sourcePackage = JSON.parse(
          await fs.readFile(sourcePackageJson, 'utf8')
        );
      }
...
async function generateMigrationReport(results) {
  ...
  if (successful.length > 0) {
    ...
    successful.forEach(r => {
      console.log(
        `   - ${r.migration.name}: ${r.migration.sourceRepo} → ${r.migration.targetPath}`
      );
    });
  }
