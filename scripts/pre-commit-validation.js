...
try {
  const projectsIndex = JSON.parse(
    fs.readFileSync('projects.index.json', 'utf8')
  );
  if (!projectsIndex.projects || !Array.isArray(projectsIndex.projects)) {
    errors.push('Invalid projects.index.json structure');
  }
} catch (err) {
