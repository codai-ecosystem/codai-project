...
mcpServers.forEach(server => {
    ...
    try {
        ...
        buildResults.issues.push(`Build failed for ${server}`);
...
// Check services synchronously for simpler script
keyServices.forEach(service => {
    serviceResults.total++;
    console.log(`  Testing: ${service.name} on port ${service.port}`);

    try {
        // Simple port check - in production would use actual HTTP request
        const isHealthy = false; // Would implement actual check
        if (isHealthy) {
...
mcpConfigs.forEach(config => {
    ...
    if (fs.existsSync(config)) {
        try {
            const packageJson = JSON.parse(fs.readFileSync(config, 'utf8'));
