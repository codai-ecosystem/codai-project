...
const SCAFFOLDED_SERVICES = [
    'wallet', 'fabricai', 'sociai', 'cumparai', 'x'
];
...
function generateCommitMessage(serviceName) {
    ...
    const serviceFeatures = {
        ...
        sociai: ['Social networking', 'AI-powered content', 'Community features', 'Real-time chat'],
...
async function pushService(serviceName) {
    ...
    try {
        ...
        if (error.message.includes('nothing to commit')) {
            ...
            console.log(chalk.red(`  ❌ Error with ${serviceName}: ${error.message}`));
            return false;
        }
    }
}
