...
async function pushService(service) {
  ...
  if (!fs.existsSync(servicePath)) {
    console.log(
      chalk.yellow(`  ⚠️  Service directory not found: ${servicePath}`)
    );
    return false;
  }
  ...
  try {
    ...
    console.log(
      chalk.green(
        `  ✅ Pushed ${service.name} to codai-ecosystem/${service.name}`
      )
    );
...
async function main() {
  console.log(
    chalk.bold.blue('🚀 Pushing Scaffolded Services to Codai Ecosystem\n')
  );
