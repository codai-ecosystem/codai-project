...
class ReleaseOrchestrator {
  ...
  async run() {
    ...
    console.log(
      `📦 Release Type: ${this.releaseType} (${RELEASE_TYPES[this.releaseType]?.description || 'custom'})`
    );
  ...
  async preReleaseChecks() {
    ...
    if (!this.dryRun) {
      try {
        ...
        if (gitStatus.trim()) {
          throw new Error(
            'Working directory is not clean. Commit or stash changes before release.'
          );
        }
      } catch (error) {
    ...
    if (
      !RELEASE_TYPES[this.releaseType] &&
      !['patch', 'minor', 'major', 'prerelease'].includes(this.releaseType)
    ) {
  ...
  async discoverServices() {
    ...
    try {
      ...
      for (const service of servicesList) {
        ...
        if (stat.isDirectory()) {
          const serviceInfo = await this.analyzeService(
            service,
            servicePath,
            'service'
          );
  ...
  async analyzeService(name, path, type) {
    ...
    try {
      ...
      return {
        ...
        hasPublishScript: !!(
          packageJson.scripts && packageJson.scripts.publish
        ),
        isPrivate: packageJson.private === true,
      };
    } catch (error) {
      console.log(
        `⚠️  ${type} ${name} missing or invalid package.json: ${error.message}`
      );
      return null;
    }
  }
  ...
  async runPreReleaseTests() {
    ...
    try {
      ...
      if (this.isCI) {
        ...
        console.log(
          '⚠️  Pre-release tests failed, continuing in development mode\n'
        );
      }
    }
  }
  ...
  async releaseService(service) {
    ...
    console.log(
      `   ${status} ${service.name}${version} (${serviceResult.duration}ms)`
    );
  ...
  async bumpVersion(service) {
    ...
    try {
      const result = await this.execCommand(
        `npm version ${this.releaseType} --no-git-tag-version`,
        {
          cwd: service.path,
        }
      );
  ...
  async createGitTag(service, version) {
    try {
      ...
      await this.execCommand(
        `git tag -a "${tagName}" -m "Release ${service.name} v${version}"`
      );
    } catch (error) {
  ...
  async generateReport() {
    ...
    const successRate =
      this.results.total > 0
        ? Math.round((this.results.released / this.results.total) * 100)
        : 0;
    ...
    Object.entries(this.results.services).forEach(([name, result]) => {
      ...
      const version =
        result.oldVersion !== result.newVersion
          ...
          : '';
      console.log(
        `   ${status} ${name} (P${result.priority})${version} - ${result.duration}ms`
      );
    ...
    const versionChanges = Object.values(this.results.services).filter(
      s => s.oldVersion !== s.newVersion
    );
    ...
    if (versionChanges.length > 0) {
      ...
      versionChanges.forEach(service => {
        console.log(
          `   ${service.name}: ${service.oldVersion} → ${service.newVersion}`
        );
      });
    }
...
if (
  process.argv[1] &&
  (process.argv[1].endsWith('release-orchestrator.js') ||
