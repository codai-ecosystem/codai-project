...
if ($criticalIssues.Count -gt 0) {
    Write-Host "`n🚨 Critical Services Needing Attention" -ForegroundColor Red
...
Write-Host "`n✅ Healthy Services" -ForegroundColor Green
Write-Host "==================" -ForegroundColor Green
foreach ($service in $healthyServices) {
    $status = if ($service.Priority -eq "Critical") { "🔥" } else { "✅" }
    Write-Host "$status $($service.Name)" -ForegroundColor Green
}

if ($issueServices.Count -gt 0) {
    Write-Host "`n⚠️  Services with Issues" -ForegroundColor Yellow
    Write-Host "=======================" -ForegroundColor Yellow
    foreach ($service in $issueServices | Where-Object { $_.Priority -ne "Critical" }) {
        Write-Host "⚠️  $($service.Name)" -ForegroundColor Yellow
        foreach ($issue in $service.Issues) {
...
foreach ($service in $criticalIssues) {
    foreach ($rec in $service.Recommendations) {
        $highPriorityActions += "🔥 $($service.Name): $rec"
    }
}

foreach ($service in $issueServices | Where-Object { $_.Priority -ne "Critical" }) {
    foreach ($rec in $service.Recommendations) {
        $mediumPriorityActions += "⚠️  $($service.Name): $rec"
    }
}

if ($highPriorityActions.Count -gt 0) {
    Write-Host "`nHigh Priority (Critical Services):" -ForegroundColor Red
    $highPriorityActions | ForEach-Object { Write-Host "  $_" -ForegroundColor Red }
}

if ($mediumPriorityActions.Count -gt 0) {
    Write-Host "`nMedium Priority:" -ForegroundColor Yellow
    $mediumPriorityActions | ForEach-Object { Write-Host "  $_" -ForegroundColor Yellow }
}
...
Write-Host "`n📋 Detailed report saved to: SERVICE_HEALTH_REPORT.json" -ForegroundColor Cyan

$endTime = Get-Date
$duration = $endTime - $startTime
Write-Host "⏱️  Health check completed in $($duration.TotalSeconds.ToString('F1')) seconds" -ForegroundColor Green

# Return summary for further processing
return @{
    HealthPercentage = $detailedResults.HealthPercentage
    CriticalIssues = $criticalIssues.Count
    TotalIssues = $issueServices.Count
