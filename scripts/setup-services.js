...
function checkGitRepository() {
  ...
  if (!result.success) {
    logStatus('This is not a git repository. Please initialize git first.', 'error');
    process.exit(1);
  }
}
...
function setupService(serviceName, options = {}) {
  ...
  execCommand(`git submodule update --init --recursive services/${serviceName}`);
...
async function main() {
  ...
  logStatus(`Setup complete! ${successCount}/${serviceNames.length} services configured.`, 'success');
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1].endsWith('setup-services.js')) {
  main().catch(error => {
