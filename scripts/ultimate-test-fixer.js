...
class UltimateTestFixer {
  ...
  async fixAllServices() {
    ...
    try {
      ...
      log(
        `   Success Rate: ${Math.round((this.fixedServices / this.totalServices) * 100)}%`,
        'cyan'
      );
  ...
  async fixService(service) {
    ...
    try {
      ...
      let testScript = '';
      let vitestConfigExists =
        fs.existsSync(path.join(service.path, 'vitest.config.js')) ||
        fs.existsSync(path.join(service.path, 'vitest.config.ts'));
      let jestConfigExists =
        fs.existsSync(path.join(service.path, 'jest.config.js')) ||
      ...
      packageJson.scripts['test:ui'] = testScript.replace(
