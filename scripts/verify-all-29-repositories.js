...
function checkRepository(repoName) {
  ...
  result.hasAgentConfig = fs.existsSync(
    path.join(repoPath, 'agent.project.json')
  );
...
async function main() {
  ...
  console.log(
    `✅ Successfully Connected: ${summary.successful}/${summary.total} (${summary.completionRate}%)`
  );
  ...
  if (summary.completionRate === 100) {
    ...
    console.log(
      '✨ The entire Codai ecosystem is properly connected and ready'
    );
