...
function checkService(serviceName) {
  ...
  try {
    files = fs
      ...
      .filter(
        file => !file.startsWith('.git') && file !== 'node_modules'
      ).length;
  } catch (error) {
    files = 0;
  }
...
function main() {
  ...
  console.log(
    `🟡 Minimal:       ${results.filter(r => r.status === 'MINIMAL').length}`
  );
  console.log(
  ...
  const percentageReady = Math.round(
    (totalNonEmpty / ALL_SERVICES.length) * 100
  );
