...
const mockAIRecommendations = [
  {
    ...
    confidence: 0.78,
    reasoning:
      'Strong momentum with volume confirmation. Technical indicators suggest continuation of uptrend. AI sentiment analysis shows positive outlook for next earnings.',
    targetPrice: 195.0,
    stopLoss: 180.0,
    timeframe: '1-2 weeks',
    riskReward: 3.2,
  ...
  },
  {
    ...
    confidence: 0.85,
    reasoning:
      'AI sector momentum accelerating. Earnings revision cycle turning positive. Data center demand exceeding expectations.',
    targetPrice: 520.0,
    stopLoss: 470.0,
    timeframe: '2-4 weeks',
    riskReward: 2.3,
  ...
  },
  {
    ...
    confidence: 0.65,
    reasoning:
      'Mixed signals from technical and fundamental analysis. Waiting for clearer direction before recommending action.',
    targetPrice: 250.0,
    stopLoss: 235.0,
    timeframe: '1-3 weeks',
    riskReward: 1.8,
  ...
  },
  {
    ...
    confidence: 0.72,
    reasoning:
      'Broad market exposure with low fees. Dollar-cost averaging opportunity in current market conditions.',
    targetPrice: 255.0,
    stopLoss: 240.0,
  ...
  recommendations: [
    {
      ...
      priority: 'high',
      description:
        'Tech sector overweight at 35%, consider reducing to target 25%',
      action:
        'Sell 20% of NVDA position and diversify into bonds or international ETFs',
  ...
  try {
    ...
    switch (type) {
      case 'recommendations':
        if (symbol) {
          data = mockAIRecommendations.filter(
            rec => rec.symbol.toLowerCase() === symbol.toLowerCase()
          );
        } else {
        ...
        data = mockAIRecommendations
          .map(rec => {
            ...
            const confidence = Math.max(
              0.1,
              Math.min(0.95, rec.confidence + (Math.random() - 0.5) * 0.1)
            );
            const aiScore = Math.max(
              1,
              Math.min(10, rec.aiScore + (Math.random() - 0.5) * 0.5)
            );
      ...
      default:
        return NextResponse.json(
          {
            error:
              'Invalid type parameter. Use: recommendations, portfolio-analysis, or signals',
          },
          { status: 400 }
        );
    }
    ...
    switch (action) {
          ...
          recommendations: [
            {
              ...
              priority: 'high',
              description:
                'Portfolio concentration in top 3 holdings exceeds 60%',
              suggestedAction:
                'Consider reducing largest positions and adding small-cap exposure',
            },
          ],
          timestamp: new Date().toISOString(),
        };
      ...
      default:
        return NextResponse.json(
          {
            error:
              'Invalid action. Use: analyze-portfolio, generate-signals, or update-preferences',
          },
          { status: 400 }
        );
    }
