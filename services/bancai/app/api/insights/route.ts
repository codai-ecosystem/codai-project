...
async function handleGetInsights(req: AuthenticatedRequest, user: AuthUser) {
  try {
    ...
    const existingInsights = await memoryService.searchFinancialMemories(
    ...
    if (existingInsights && existingInsights.length > 0) {
      ...
      if (type) {
        filteredInsights = existingInsights.filter(
          insight => insight.metadata?.insightType === type
        );
      }

      const insights = filteredInsights.slice(0, limit).map(entry => ({
        id: entry.id,
        userId: user.id,
        type: entry.metadata?.insightType || 'spending_pattern',
        title:
          entry.content?.split(' - ')[0]?.replace('AI Insight: ', '') ||
          'Financial Insight',
        description:
          entry.content?.split(' - ')[1] ||
          entry.content ||
    ...
    const mockInsights: AIInsight[] = [
      {
        ...
        description:
          "You've spent $180 on coffee this month, 23% more than last month. Consider brewing at home to save $120/month.",
        confidence: 0.92,
        actionable: true,
        data: {
          category: 'food',
      ...
      },
      {
        ...
        description:
          'Based on your income pattern, you can safely save an additional $300/month by setting up automatic transfers.',
        confidence: 0.87,
        actionable: true,
        data: {
      ...
      },
      {
        ...
        description:
          'Your emergency fund is well-established. Consider diversifying with low-cost index funds.',
        confidence: 0.78,
        actionable: true,
        data: {
