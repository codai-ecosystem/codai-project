...
async function handleGetTransactions(req: AuthenticatedRequest, user: AuthUser) {
  try {
    ...
    const existingTransactionsResult = await memoryService.searchFinancialMemories('transactions user financial data');
    ...
    if (existingTransactionsResult && existingTransactionsResult.length > 0) {
      ...
      if (transactionMemory && transactionMemory.content) {
        try {
          const transactionsData = JSON.parse(transactionMemory.content.replace('User transactions: ', ''));
          ...
          console.log('Error parsing existing transactions from memory, creating new ones');
        }
      }
    }
