...
export const PortfolioManager: React.FC = () => {
    ...
    const [selectedTab, setSelectedTab] = useState<'overview' | 'holdings' | 'ai-insights' | 'analytics'>('overview');
        ...
        const mockRecommendations: AIRecommendation[] = [
            {
                ...
                symbol: 'MSFT',
                confidence: 0.85,
                reasoning: 'Strong AI fundamentals and cloud growth potential. Undervalued relative to earnings growth.',
                expectedReturn: 12.5,
                riskLevel: 'medium',
            ...
            },
            {
                ...
                symbol: 'BTC',
                confidence: 0.72,
                reasoning: 'Crypto allocation exceeds target range. Consider taking profits and rebalancing into bonds.',
                expectedReturn: 8.2,
                riskLevel: 'high',
            ...
            },
            {
                ...
                symbol: 'VTI',
                confidence: 0.91,
                reasoning: 'Strong long-term diversification anchor. Maintain position for continued market exposure.',
                expectedReturn: 7.8,
                riskLevel: 'low',
                timeframe: '12+ months'
            }
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xl font-semibold">{formatCurrency(portfolio.totalReturn)}</p>
                        ...
                        <div className="space-y-2">
                            ...
                            <Badge variant={portfolio.riskScore <= 3 ? 'success' : portfolio.riskScore <= 7 ? 'warning' : 'destructive'}>
                                {portfolio.riskScore <= 3 ? 'Conservative' : portfolio.riskScore <= 7 ? 'Moderate' : 'Aggressive'}
                            </Badge>
                        </div>

                        <div className="space-y-2">
                            <p className="text-sm text-gray-500">Diversification</p>
                            <p className="text-xl font-semibold">{portfolio.diversificationScore}/10</p>
                            <Badge variant={portfolio.diversificationScore >= 8 ? 'success' : portfolio.diversificationScore >= 6 ? 'warning' : 'destructive'}>
                                {portfolio.diversificationScore >= 8 ? 'Excellent' : portfolio.diversificationScore >= 6 ? 'Good' : 'Needs Work'}
                            </Badge>
                        </div>
                    </div>
                </CardContent>
                            ...
                            {portfolio.holdings.map((holding) => (
                                <div key={holding.id} className="flex items-center justify-between p-4 border rounded-lg">
                                    ...
                                    <div className="text-right space-y-1">
                                        ...
                                        <p className="text-sm text-gray-500">{holding.shares} shares @ {formatCurrency(holding.currentPrice)}</p>
                                        ...
                                        <p className="text-xs text-gray-400">{holding.allocationPercent.toFixed(2)}% allocation</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                                ...
                                <div key={rec.id} className="p-4 border rounded-lg space-y-3">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <Badge variant={getRiskColor(rec.riskLevel)} className="text-xs">
                                                {rec.riskLevel} risk
                                            </Badge>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-sm text-gray-500">Confidence: {(rec.confidence * 100).toFixed(0)}%</p>
                                            <p className="text-sm text-green-600">Expected: +{rec.expectedReturn}%</p>
                                        </div>
                                    </div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between">
                                    <span className="text-sm text-gray-500">Information Ratio</span>
