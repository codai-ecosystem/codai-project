...
export default function OptionsTrading() {
    ...
    const loadExpirations = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/options?type=expirations&symbol=${selectedSymbol}&userId=demo-user`);
    ...
    const loadOptionsChain = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/options?type=chain&symbol=${selectedSymbol}&expiration=${selectedExpiration}&userId=demo-user`);
            const result = await response.json();
    ...
    const loadStrategies = async () => {
        try {
            const response = await fetch(`/api/options?type=strategies&symbol=${selectedSymbol}&userId=demo-user`);
            const result = await response.json();
    ...
    const loadVolatilityData = async () => {
        try {
            const response = await fetch(`/api/options?type=volatility&symbol=${selectedSymbol}&userId=demo-user`);
            const result = await response.json();
    ...
    const getStrategyBadgeColor = (strategy: string) => {
        switch (strategy) {
            ...
            case 'hedge': return 'bg-red-100 text-red-800';
    ...
    const OptionsChainTable = ({ contracts, type }: { contracts: OptionContract[], type: 'calls' | 'puts' }) => (
                ...
                <tbody>
                    {contracts.map((contract, index) => (
                        ...
                        >
                            ...
                            <td className="p-2">{formatPercent(contract.impliedVolatility)}</td>
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Options Trading</h2>
                    <p className="text-gray-600 dark:text-gray-300">Advanced options analysis and strategy tools</p>
                </div>
                    ...
                    {optionsChain && (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <Card>
                                ...
                                <CardContent>
                                    <OptionsChainTable contracts={optionsChain.calls} type="calls" />
                                </CardContent>
                            </Card>
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <OptionsChainTable contracts={optionsChain.puts} type="puts" />
                                </CardContent>
                            </Card>
                        </div>
                    )}
                                ...
                                <CardHeader>
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center space-x-1">
                                            <Badge className={getStrategyBadgeColor(strategy.strategy)}>
                                                {strategy.strategy}
                                            </Badge>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div>
                                            ...
                                            <span className="text-green-600">{formatCurrency(strategy.maxProfit)}</span>
                                        </div>
                                        <div>
                                            <div className="flex items-center space-x-1">
                                            ...
                                            <span className="text-red-600">{formatCurrency(strategy.maxLoss)}</span>
                                        </div>
                                    </div>
                                            ...
                                            <div className="flex items-center space-x-1">
                                                <div className="w-20 bg-gray-200 rounded-full h-2">
                                                    <div
                                                        ...
                                                        style={{ width: `${(strategy.aiRating / 10) * 100}%` }}
                                                    />
                                                </div>
                                                <span className="font-medium">{strategy.aiRating.toFixed(1)}/10</span>
                                            </div>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Success Probability:</span>
                                            <span className="font-medium">{formatPercent(strategy.probability)}</span>
                                        </div>
                                    </div>

                                    <div className="pt-2 border-t">
                                        <p className="text-xs text-gray-600">{strategy.recommendation}</p>
                                    </div>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <div className="text-2xl font-bold">{formatPercent(volatilityData.impliedVolatility.current)}</div>
                                        </div>
                                        <div>
                                            <Label>IV Rank</Label>
                                            <div className="text-2xl font-bold">{volatilityData.impliedVolatility.rank}%</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 text-sm">
                                            <div className="flex justify-between">
                                                <span>30-day:</span>
                                                <span>{formatPercent(volatilityData.historicalVolatility['30day'])}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>60-day:</span>
                                                <span>{formatPercent(volatilityData.historicalVolatility['60day'])}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>90-day:</span>
                                                <span>{formatPercent(volatilityData.historicalVolatility['90day'])}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>Annualized:</span>
                                                <span>{formatPercent(volatilityData.historicalVolatility.annualized)}</span>
                                            </div>
                                        </div>
                                    </div>
                                </CardContent>
                                        ...
                                        {volatilityData.volatilitySkew.map((point, index) => (
                                            <div key={index} className="flex justify-between items-center">
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    ...
                                                    <span className="text-sm">{formatPercent(point.iv)}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Create custom options strategies</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div className="border rounded-lg p-3 space-y-2">
                                            {strategyLegs.map((leg, index) => (
                                                <div key={index} className="flex items-center justify-between text-sm">
                                                    <span>{leg.action} {leg.quantity} {leg.type} @ ${leg.strike}</span>
                        ...
                        {strategyAnalysis && (
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Risk/reward analysis results</CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center space-x-2">
                                            <div className="w-full bg-gray-200 rounded-full h-2">
                                                <div
                                                    className="bg-blue-600 h-2 rounded-full"
                                                    style={{ width: `${(strategyAnalysis.aiRating / 10) * 100}%` }}
                                                />
                                            </div>
                                            <span className="font-medium">{strategyAnalysis.aiRating.toFixed(1)}/10</span>
                                        </div>
                                    </div>

                                    <div>
                                        <Label>Success Probability</Label>
                                        <div className="text-lg font-bold">{formatPercent(strategyAnalysis.probability)}</div>
                                    </div>
                                    ...
                                    <div className="pt-2 border-t">
                                        ...
                                        <p className="text-sm text-gray-600">{strategyAnalysis.recommendation}</p>
                                    </div>
                                </CardContent>
                            </Card>
                        )}
