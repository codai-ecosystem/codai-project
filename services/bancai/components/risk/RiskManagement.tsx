...
export const RiskManagement: React.FC = () => {
    ...
    const [selectedTab, setSelectedTab] = useState<'overview' | 'alerts' | 'stress-test' | 'hedging'>('overview');
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        // Mock risk management data
        const mockRiskMetrics: RiskMetrics = {
            portfolioValue: 125840.50,
            totalRisk: 18750.25,
            riskScore: 7.2,
            riskLevel: 'medium',
            valueAtRisk: {
        ...
        const mockAlerts: RiskAlert[] = [
            {
                ...
                description: 'Your portfolio has 35% allocation in technology sector, exceeding recommended maximum of 30%.',
                recommendation: 'Consider rebalancing to reduce sector concentration by selling some tech positions.',
            ...
            },
            {
                ...
                description: 'Daily Value at Risk has exceeded your set threshold of $2,000.',
                recommendation: 'Review position sizes and consider hedging strategies to reduce risk exposure.',
            ...
            },
            {
                ...
                recommendation: 'Schedule automatic rebalancing or manually adjust positions to target weights.',
        ...
        const mockStressTests: StressTestScenario[] = [
            ...
            },
            {
                ...
                description: 'Rapid interest rate increases affecting all asset classes',
                marketDropPercent: -25,
                portfolioImpact: -28750.00,
                impactPercent: -22.8,
                probability: 0.25,
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div className="space-y-2">
                            ...
                            <p className="text-2xl font-bold">{formatCurrency(riskMetrics.portfolioValue)}</p>
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xl font-semibold">{riskMetrics.riskScore}/10</p>
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-sm text-gray-500">
                                {((riskMetrics.valueAtRisk.oneDay / riskMetrics.portfolioValue) * 100).toFixed(2)}% of portfolio
                            </p>
                        </div>

                        <div className="space-y-2">
                            <p className="text-sm text-gray-500">Diversification</p>
                            <p className="text-xl font-semibold">{riskMetrics.diversificationScore}/10</p>
                            <Badge variant={riskMetrics.diversificationScore >= 8 ? 'success' : riskMetrics.diversificationScore >= 6 ? 'warning' : 'destructive'}>
                                {riskMetrics.diversificationScore >= 8 ? 'Excellent' : riskMetrics.diversificationScore >= 6 ? 'Good' : 'Poor'}
                            </Badge>
                        </div>
                    </div>
                    ...
                    <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t">
                        ...
                        <div className="text-center">
                            ...
                            <p className="font-semibold text-red-600">{formatPercent(riskMetrics.maxDrawdown)}</p>
                        </div>
                        <div className="text-center">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-gray-500">1 Week VaR (95%)</span>
                                    <span className="font-semibold text-red-600">
                                        {formatCurrency(riskMetrics.valueAtRisk.oneWeek)}
                                    </span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-gray-500">1 Month VaR (95%)</span>
                                    <span className="font-semibold text-red-600">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-gray-500">Annual Volatility</span>
                                ...
                                >
                                    <div className="flex items-start justify-between">
                                        <div className="flex items-start space-x-3">
                                            <AlertTriangle className={`h-5 w-5 mt-0.5 ${alert.type === 'danger' ? 'text-red-500' :
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex items-center space-x-2">
                                                    ...
                                                    <Badge variant={getAlertTypeColor(alert.type, alert.severity)}>
                                                ...
                                                <p className="text-sm font-medium text-blue-600">{alert.recommendation}</p>
                                ...
                                <div key={scenario.id} className="p-4 border rounded-lg space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div>
                                            ...
                                            <p className="font-semibold text-red-600">{formatPercent(scenario.marketDropPercent)}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-500">Portfolio Impact</p>
                                            <p className="font-semibold text-red-600">{formatCurrency(scenario.portfolioImpact)}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-500">Impact %</p>
                                            <p className="font-semibold text-red-600">{formatPercent(scenario.impactPercent)}</p>
                                        </div>
                                        <div>
                                ...
                                <div key={strategy.id} className="p-4 border rounded-lg space-y-3">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <Badge variant={getRecommendationColor(strategy.recommendation)}>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div>
                                            ...
                                            <p className="font-semibold">{formatCurrency(strategy.cost)}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-500">Effectiveness</p>
                                            <p className="font-semibold">{(strategy.effectiveness * 100).toFixed(0)}%</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-500">Protection Level</p>
                                            <p className="font-semibold">{strategy.protectionLevel}%</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-500">Cost/Benefit</p>
                                            <p className="font-semibold">
                                                {((strategy.effectiveness * strategy.protectionLevel) / (strategy.cost / 1000)).toFixed(1)}
                                            </p>
                                        </div>
                                    </div>
