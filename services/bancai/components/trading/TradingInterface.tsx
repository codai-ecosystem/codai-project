...
import { TrendingUp, TrendingDown, Zap, AlertTriangle, DollarSign, Activity } from 'lucide-react';
...
export const TradingInterface: React.FC = () => {
        ...
        const mockAISignals: AISignal[] = [
            {
                ...
                signal: 'buy',
                confidence: 0.78,
                reasoning: 'Strong momentum with volume confirmation. Technical indicators suggest continuation of uptrend.',
                targetPrice: 195.00,
                stopLoss: 180.00,
                timeframe: '1-2 weeks',
                riskReward: 3.2
            },
            {
                ...
                signal: 'strong_buy',
                confidence: 0.85,
                reasoning: 'AI sector momentum accelerating. Earnings revision cycle turning positive.',
                targetPrice: 520.00,
                stopLoss: 470.00,
                timeframe: '2-4 weeks',
                riskReward: 2.3
            },
            {
                ...
                signal: 'hold',
                confidence: 0.65,
                reasoning: 'Mixed signals. Waiting for clearer direction before recommending action.',
                targetPrice: 250.00,
                stopLoss: 235.00,
                timeframe: '1-3 weeks',
                riskReward: 1.8
    ...
    const getSignalIcon = (signal: string) => {
        switch (signal) {
            case 'strong_buy': return <TrendingUp className="h-4 w-4 text-green-600" />;
            case 'buy': return <TrendingUp className="h-4 w-4 text-blue-600" />;
            case 'hold': return <Activity className="h-4 w-4 text-gray-600" />;
            case 'sell': return <TrendingDown className="h-4 w-4 text-orange-600" />;
            case 'strong_sell': return <TrendingDown className="h-4 w-4 text-red-600" />;
            default: return <Activity className="h-4 w-4" />;
        }
    };

    const currentMarketData = marketData.find(item => item.symbol === selectedSymbol);
                    ...
                    <CardContent>
                        <div className="space-y-4">
                            {marketData.map((item) => (
                                <div
                                    key={item.symbol}
                                    className={`p-4 border rounded-lg cursor-pointer transition-colors ${selectedSymbol === item.symbol ? 'border-blue-500 bg-blue-50' : 'hover:bg-gray-50'
                                        }`}
                        ...
                        <div className="space-y-4">
                            {positions.map((position) => (
                                <div key={position.id} className="flex items-center justify-between p-4 border rounded-lg">
                                    ...
                                    <div className="text-right">
                                        <p className={`font-semibold ${getChangeColor(position.unrealizedPL)}`}>
                                            {formatCurrency(position.unrealizedPL)}
                                        </p>
                                        <p className={`text-sm ${getChangeColor(position.unrealizedPL)}`}>
                            ...
                            <div className="pt-4 border-t space-y-2 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium">
                                        {formatCurrency((orderType === 'market' ? currentMarketData.price : price) * quantity)}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span>Buying Power:</span>
                                    <span className="text-green-600">{formatCurrency(50000)}</span>
                                </div>
                            </div>
                        )}
                    </CardContent>
                    ...
                    <CardContent>
                        <div className="space-y-4">
                            {aiSignals.map((signal) => (
                                <div key={signal.id} className="p-4 border rounded-lg space-y-3">
                                    <div className="flex items-center justify-between">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-xs">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-medium">{signal.riskReward.toFixed(1)}:1</p>
                                        </div>
                                        <div>
