    ...
    switch (type) {
      ...
      default:
        return NextResponse.json(
          {
            error:
              'Invalid type parameter. Use: templates, deployments, or training-jobs',
          },
          { status: 400 }
        );
    }
  ...
  try {
    ...
    switch (action) {
      ...
      case 'start-training':
        ...
        const trainTemplate = mockModelTemplates.find(
          t => t.id === trainTemplateId
        );
        if (!trainTemplate) {
          return NextResponse.json(
            { error: 'Template not found' },
      ...
      default:
        return NextResponse.json(
          {
            error:
              'Invalid action. Use: deploy-model, manage-deployment, start-training, or create-template',
          },
          { status: 400 }
        );
    }
