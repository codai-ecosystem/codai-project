...
export default function AIServiceOrchestrator() {
    ...
    const loadServices = async () => {
        try {
            setLoading(true);
            const response = await fetch('/api/ai-services?type=services&userId=demo-user');
    ...
    const loadRequests = async () => {
        try {
            const response = await fetch('/api/ai-services?type=requests&userId=demo-user&limit=50');
            const result = await response.json();
    ...
    const loadPipelines = async () => {
        try {
            const response = await fetch('/api/ai-services?type=pipelines&userId=demo-user');
            const result = await response.json();
    ...
    const loadMetrics = async () => {
        try {
            const response = await fetch('/api/ai-services?type=metrics&userId=demo-user');
            const result = await response.json();
    ...
    const getCategoryIcon = (category: string) => {
        switch (category) {
            case 'nlp': return <MessageSquare className="w-5 h-5 text-blue-600" />;
            case 'vision': return <Image className="w-5 h-5 text-purple-600" />;
            case 'code': return <Code className="w-5 h-5 text-green-600" />;
            case 'analytics': return <BarChart3 className="w-5 h-5 text-orange-600" />;
            case 'automation': return <Zap className="w-5 h-5 text-yellow-600" />;
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">AI Service Orchestrator</h2>
                    <p className="text-gray-600 dark:text-gray-300">Manage and orchestrate AI services across the ecosystem</p>
                </div>
                    ...
                    <CardContent className="p-4">
                        <div className="flex items-center space-x-2">
                            ...
                            <div>
                                ...
                                <p className="text-xl font-bold">{(metrics.successRate * 100).toFixed(1)}%</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div>
                                            ...
                                            <span className="ml-1">{service.requestCount.toLocaleString()}</span>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <span className="ml-1">{(service.successRate * 100).toFixed(1)}%</span>
                                        </div>
                                        <div>
                                            <span className="font-medium">Cost:</span>
                                            <span className="ml-1">{formatCurrency(service.cost)}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex justify-between text-sm">
                                            <span>Last Used:</span>
                                            <span>{new Date(service.lastUsed).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                    ...
                                    <tbody>
                                        {requests.slice(0, 20).map((request) => (
                                            <tr key={request.id} className="border-b hover:bg-gray-50">
                                                <td className="p-2 font-medium">{request.serviceName}</td>
                                                <td className="p-2">
                                                ...
                                                <td className="p-2">{new Date(request.requestTime).toLocaleString()}</td>
                                                <td className="p-2">
                            ...
                            <Card key={pipeline.id}>
                                <CardHeader>
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Badge className={pipeline.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}>
                                            {pipeline.status}
                                        </Badge>
                                    </div>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div>
                                            ...
                                            <span className="ml-1 text-green-600">{pipeline.successCount}</span>
                                        </div>
                                        <div>
                                            <span className="font-medium">Failures:</span>
                                            <span className="ml-1 text-red-600">{pipeline.failureCount}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex justify-between text-sm">
                                        <span>Last Run:</span>
                                        <span>{pipeline.lastRun ? new Date(pipeline.lastRun).toLocaleDateString() : 'Never'}</span>
                                    </div>
