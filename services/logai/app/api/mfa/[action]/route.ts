...
async function handleGenerateSecret(request: NextRequest, token: any): Promise<NextResponse> {
  try {
...
// Enable MFA
async function handleEnableMFA(request: NextRequest, token: any): Promise<NextResponse> {
  try {
    const userId = token.userId as string
    ...
    const result = await mfaService.enableMFA(userId, secret, mfaToken, clientIP)
...
// Disable MFA
async function handleDisableMFA(request: NextRequest, token: any): Promise<NextResponse> {
  try {
    const userId = token.userId as string
    const clientIP = getClientIP(request)
    const body = await request.json()
...
// Get MFA status
async function handleGetMFAStatus(request: NextRequest, token: any): Promise<NextResponse> {
  try {
    const userId = token.userId as string
...
async function handleRegenerateBackupCodes(request: NextRequest, token: any): Promise<NextResponse> {
  try {
    const userId = token.userId as string
    const clientIP = getClientIP(request)
    const body = await request.json()
    ...
    const result = await mfaService.regenerateBackupCodes(userId, mfaToken, clientIP)
...
export async function GET(request: NextRequest, { params }: { params: { action: string } }) {
  ...
  switch (action) {
    case "generate-secret":
      return withAPIMiddleware(request, handleGenerateSecret, { requireAuth: true })
    case "status":
      return withAPIMiddleware(request, handleGetMFAStatus, { requireAuth: true })
    default:
...
export async function POST(request: NextRequest, { params }: { params: { action: string } }) {
  ...
  switch (action) {
    ...
    case "disable":
      return withAPIMiddleware(request, handleDisableMFA, { requireAuth: true })
    case "regenerate-backup-codes":
      return withAPIMiddleware(request, handleRegenerateBackupCodes, { requireAuth: true })
    default:
      return NextResponse.json({ error: "Invalid action" }, { status: 400 })
  }
}
