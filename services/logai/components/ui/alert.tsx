  ...
  'relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-gray-950 dark:[&>svg]:text-gray-50',
  {
    variants: {
      variant: {
        ...
        info:
          'border-blue-500/50 text-blue-900 dark:border-blue-500 [&>svg]:text-blue-600 dark:border-blue-900/50 dark:text-blue-50 dark:dark:border-blue-900 dark:[&>svg]:text-blue-400 bg-blue-50 dark:bg-blue-950/30',
      },
    },
    defaultVariants: {
      variant: 'default',
...
const AlertTriangleIcon = ({ className, ...props }: React.SVGProps<SVGSVGElement>) => (
  <svg
    className={cn('h-4 w-4', className)}
    fill="none"
    stroke="currentColor"
...
const ErrorAlert = ({ title, description, ...props }: {
  title?: string;
  description: string;
...
const SuccessAlert = ({ title, description, ...props }: {
  title?: string;
  description: string;
} & React.HTMLAttributes<HTMLDivElement>) => (
...
const InfoAlert = ({ title, description, ...props }: {
  title?: string;
  description: string;
} & React.HTMLAttributes<HTMLDivElement>) => (
