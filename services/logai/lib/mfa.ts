...
export class MFAService {
   ...
   */
  async enableMFA(userId: string, secret: string, token: string, ipAddress?: string): Promise<{
    success: boolean
    backupCodes?: string[]
    error?: string
  }> {
   ...
   */
  async disableMFA(userId: string, token: string, ipAddress?: string): Promise<{
    success: boolean
    error?: string
  }> {
    try {
   ...
   * Verify backup code
   */
  async verifyBackupCode(userId: string, backupCode: string, ipAddress?: string): Promise<{
    success: boolean
    error?: string
  }> {
    try {
   ...
   */
  async regenerateBackupCodes(userId: string, token: string, ipAddress?: string): Promise<{
    success: boolean
    backupCodes?: string[]
    error?: string
  }> {
  ...
  async getMFAStatus(userId: string): Promise<{
    ...
    try {
      ...
      return { enabled: false, backupCodesCount: 0, error: "Failed to get MFA status" }
    }
  }
}
