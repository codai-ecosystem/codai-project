...
export async function checkRateLimit(
  ...
  key: string,
  points: number = 1
): Promise<{ allowed: boolean; totalHits: number; totalTime: number; remainingPoints: number }> {
  try {
...
export async function getRateLimitInfo(
  limiter: RateLimiterRedis,
  key: string
): Promise<{ totalHits: number; remainingPoints: number; msBeforeNext: number } | null> {
  try {
    await ensureConnection()
...
export async function resetRateLimit(limiter: RateLimiterRedis, key: string): Promise<void> {
  try {
    await ensureConnection()
