...
export async function withRateLimit(
  ...
  try {
      ...
      return new NextResponse(
        JSON.stringify({ error: "Rate limit exceeded" }),
        {
          ...
          headers: {
            ...
            "X-RateLimit-Reset": new Date(Date.now() + rateLimit.totalTime).toISOString(),
          }
        }
      )
    }
    ...
    response.headers.set("X-RateLimit-Remaining", rateLimit.remainingPoints.toString())
...
export async function withAPIMiddleware(
  ...
  if (request.method === "OPTIONS") {
    ...
    return withCORS(withSecurityHeaders(response), request.headers.get("origin") || undefined)
  }
