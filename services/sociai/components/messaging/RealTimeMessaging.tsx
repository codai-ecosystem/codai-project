...
export default function RealTimeMessaging() {
    ...
    const loadMessages = async (conversationId: string) => {
        try {
            const response = await fetch(`/api/social/messaging/conversations/${conversationId}/messages`);
    ...
    const simulateTyping = () => {
        const randomUser = mockOnlineUsers[Math.floor(Math.random() * mockOnlineUsers.length)];
    ...
    const mockConversations: ChatConversation[] = [
        {
            ...
            type: 'direct',
            participants: [
                { id: 'current-user', name: 'You', avatar: '/api/placeholder/32/32', status: 'online' },
                { id: 'alex', name: 'Alex Rodriguez', avatar: '/api/placeholder/32/32', status: 'online' }
            ],
            lastMessage: {
        ...
        },
        {
            ...
            participants: [
                { id: 'current-user', name: 'You', avatar: '/api/placeholder/32/32', status: 'online' },
                { id: 'alex', name: 'Alex Rodriguez', avatar: '/api/placeholder/32/32', status: 'online' },
                { id: 'sarah', name: 'Sarah Chen', avatar: '/api/placeholder/32/32', status: 'away' },
                { id: 'mike', name: 'Mike Johnson', avatar: '/api/placeholder/32/32', status: 'offline' }
            ],
            lastMessage: {
    ...
    const mockMessages: ChatMessage[] = [
        {
            ...
            senderAvatar: '/api/placeholder/32/32',
            content: 'Hey! I saw your latest post about the AI social platform. Really impressive work!',
        ...
        },
        {
            ...
            senderName: 'You',
            senderAvatar: '/api/placeholder/32/32',
            content: 'Thanks! It\'s been quite a journey. The analytics features are my favorite part.',
        ...
        },
        {
            ...
            senderName: 'Alex Rodriguez',
            senderAvatar: '/api/placeholder/32/32',
            content: 'I\'d love to collaborate on something similar. Are you open to discussing a potential partnership?',
        ...
        },
        {
            ...
            senderName: 'You',
            senderAvatar: '/api/placeholder/32/32',
            content: 'Absolutely! I\'m always interested in collaborating with talented developers. What did you have in mind?',
            timestamp: new Date(Date.now() - 300000).toISOString(),
            type: 'text',
            isRead: true
        }
            ...
            <div className="bg-white rounded-lg shadow-sm border overflow-hidden" style={{ height: '600px' }}>
                <div className="flex h-full">
                    ...
                    <div className="w-80 border-r border-gray-200 flex flex-col">
                        ...
                        {/* Online Users */}
                        <div className="p-4 border-b border-gray-200">
                            <h3 className="text-sm font-medium text-gray-900 mb-3">Online Now</h3>
                            <div className="flex space-x-2 overflow-x-auto">
                                {onlineUsers.filter(user => user.status === 'online').map((user) => (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs text-gray-600 truncate w-full text-center">{user.name.split(' ')[0]}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
                                ...
                                <div className="divide-y divide-gray-100">
                                    {filteredConversations.map((conversation) => (
                                        <button
                                            ...
                                            className={`w-full p-4 text-left hover:bg-gray-50 transition-colors ${activeConversation === conversation.id ? 'bg-blue-50 border-r-2 border-blue-500' : ''
                                                }`}
                                        >
                                            <div className="flex items-center space-x-3">
                                                <div className="relative">
                                                    ...
                                                    {conversation.type === 'direct' && (
                                                        <div className={`absolute -bottom-1 -right-1 w-3 h-3 border-2 border-white rounded-full ${conversation.participants.find(p => p.id !== 'current-user')?.status === 'online'
                                                                ? 'bg-green-500'
                                                                : conversation.participants.find(p => p.id !== 'current-user')?.status === 'away'
                                                                    ? 'bg-yellow-500'
                                                                    : 'bg-gray-400'
                                                            }`}></div>
                                                    )}
                            ...
                            <>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">
                                                {activeConv.type === 'group'
                                                    ...
                                                    : activeConv.participants.find(p => p.id !== 'current-user')?.status === 'online'
                                                        ? 'Online now'
                                                        : 'Last seen recently'
                                                }
                                            </p>
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                    {messages.map((message) => (
                                        ...
                                        >
                                            <div className={`flex items-start space-x-2 max-w-xs lg:max-w-md ${message.senderId === 'current-user' ? 'flex-row-reverse space-x-reverse' : ''
                                                }`}>
                                                <img
                                                ...
                                                />
                                                <div>
                                                    <div className={`rounded-lg px-4 py-2 ${message.senderId === 'current-user'
