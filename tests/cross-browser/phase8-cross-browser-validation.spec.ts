...
// Service configuration for cross-browser testing
const SERVICES = [
    {
        ...
        port: 4003,
        type: 'api_gateway',
        expectedTitle: 'CODAI Gateway Health Check - System Status Dashboard',
    ...
    },
    {
        ...
        expectedTitle: /Admin Dashboard|CODAI Admin/,
    ...
    },
    {
        ...
        port: 4004,
        type: 'authentication',
        expectedTitle: /ID Service|Authentication|Login/,
    ...
    },
    {
        ...
        port: 4008,
        type: 'coordination',
        expectedTitle: /Hub|Dashboard|Service/,
...
async function validateBrowserCompatibility(page: Page, service: any): Promise<any> {
    ...
    try {
        ...
        const titleMatch = typeof service.expectedTitle === 'string'
            ? title.includes(service.expectedTitle)
            : service.expectedTitle.test(title);

        if (titleMatch) {
            result.compatibility += 15;
        ...
        // CSS compatibility and responsive design
        try {
...
async function validateResponsiveDesign(page: Page, service: any, breakpoint: any): Promise<any> {
    ...
    try {
        ...
        const layoutMetrics = await page.evaluate(() => {
            ...
            const responsiveElements = {
                ...
                responsiveImages: document.querySelectorAll('img[style*="max-width"], .img-responsive, .img-fluid').length,
    ...
    const totalScore = (result.layoutScore + result.usabilityScore + result.performanceScore) / 3;
...
// Phase 8.1: Browser Compatibility Testing
test.describe('Phase 8: Cross-Browser & Device Testing - Browser Compatibility', () => {
    ...
    SERVICES.forEach(service => {
        test(`Browser Compatibility - ${service.name}`, async ({ page, browserName }) => {
            ...
            console.log(`📊 Compatibility: ${result.compatibility}% | Responsive: ${result.responsiveDesign}% | Functionality: ${result.functionality}% | Performance: ${result.performance}%`);

            // Determine grade
            let grade = 'F';
            if (result.overallScore >= 90) grade = 'A+';
            else if (result.overallScore >= 85) grade = 'A';
            else if (result.overallScore >= 80) grade = 'B+';
            else if (result.overallScore >= 75) grade = 'B';
            else if (result.overallScore >= 70) grade = 'C+';
            else if (result.overallScore >= 65) grade = 'C';
            ...
            if (service.type === 'admin_interface' || service.type === 'authentication') {
                expect(result.overallScore, `Critical service ${service.name} should have excellent browser compatibility`).toBeGreaterThan(75);
            }
        });
    });
});

// Phase 8.2: Responsive Design Testing
test.describe('Phase 8: Cross-Browser & Device Testing - Responsive Design', () => {
    ...
    SERVICES.forEach(service => {
        BREAKPOINTS.forEach(breakpoint => {
            test(`Responsive Design - ${service.name} at ${breakpoint.name}`, async ({ page }) => {
                ...
                if (!isValidResponse) {
                    test.skip(`Service ${service.name} not available`);
                    return;
                }
                ...
                console.log(`${grade}: ${service.name} at ${breakpoint.name} (Score: ${result.overallScore}%)`);
                console.log(`   Overall Score: ${result.overallScore}%`);
                ...
                if (breakpoint.name === 'mobile') {
                    expect(result.usabilityScore, `${service.name} should be mobile-friendly`).toBeGreaterThan(60);
                }
            });
        });
    });
...
// Phase 8.3: Cross-Browser Performance Testing
test.describe('Phase 8: Cross-Browser & Device Testing - Performance Across Browsers', () => {
    ...
    test('Cross-Browser Performance Comparison', async ({ page, browserName }) => {
        ...
        for (const service of SERVICES) {
            ...
            try {
                ...
                if (loadTime < 1000) performanceGrade = 'A+';
                else if (loadTime < 2000) performanceGrade = 'A';
                else if (loadTime < 3000) performanceGrade = 'B';
                else if (loadTime < 5000) performanceGrade = 'C';
                else if (loadTime < 8000) performanceGrade = 'D';

                console.log(`   ${performanceGrade}: ${service.name} (${loadTime}ms)`);
                console.log(`      TTFB: ${Math.round(metrics.timeToFirstByte)}ms | DOM Interactive: ${Math.round(metrics.domInteractive)}ms`);

                // Performance assertions
                expect(loadTime, `${service.name} should load within 8 seconds on ${browserName}`).toBeLessThan(8000);

                if (service.type === 'authentication' || service.type === 'admin_interface') {
                    expect(loadTime, `Critical service ${service.name} should load quickly on ${browserName}`).toBeLessThan(5000);
                }

            } catch (error) {
                console.log(`   ❌ ${service.name}: Performance test failed - ${error.message}`);
            }
        }
        ...
        if (performanceResults.length > 0) {
            ...
            console.log(`\n📊 ${browserName} Performance Summary:`);
            console.log(`   Average Load Time: ${Math.round(avgLoadTime)}ms`);
            console.log(`   Services Tested: ${performanceResults.length}/${SERVICES.length}`);

            // Browser performance should be reasonable
            expect(avgLoadTime, `Average performance on ${browserName} should be acceptable`).toBeLessThan(6000);
        }
    });
});

// Phase 8.4: Comprehensive Cross-Browser Assessment
test.describe('Phase 8: Cross-Browser & Device Testing - Comprehensive Assessment', () => {
    ...
    test('Comprehensive Cross-Browser Compatibility Assessment', async ({ page, browserName }) => {
        ...
        for (const service of SERVICES) {
            ...
            try {
                ...
                // Test at mobile breakpoint for responsive validation
                ...
                const serviceResult = {
                    ...
                    mobileResponsive: mobileResult.overallScore,
                    desktopResponsive: desktopResult.overallScore,
                    issues: compatibilityResult.issues + mobileResult.issues + desktopResult.issues,
                    criticalIssues: compatibilityResult.criticalIssues + mobileResult.criticalIssues + desktopResult.criticalIssues,
                    overallScore: Math.round((compatibilityResult.overallScore + mobileResult.overallScore + desktopResult.overallScore) / 3)
                };

                assessmentResults.push(serviceResult);
                totalScore += serviceResult.overallScore;
                totalIssues += serviceResult.issues;
                criticalIssues += serviceResult.criticalIssues;

                // Determine grade
                let grade = 'F';
                if (serviceResult.overallScore >= 90) grade = 'A+';
                else if (serviceResult.overallScore >= 85) grade = 'A';
                else if (serviceResult.overallScore >= 80) grade = 'B+';
                else if (serviceResult.overallScore >= 75) grade = 'B';
                else if (serviceResult.overallScore >= 70) grade = 'C+';
                else if (serviceResult.overallScore >= 65) grade = 'C';
                else if (serviceResult.overallScore >= 60) grade = 'D';

                console.log(`   ${grade}: ${service.name}`);
                console.log(`      Overall Score: ${serviceResult.overallScore}%`);
                console.log(`      Compatibility: ${serviceResult.compatibility}% | Mobile: ${serviceResult.mobileResponsive}% | Desktop: ${serviceResult.desktopResponsive}%`);
                console.log(`      Issues: ${serviceResult.issues} total, ${serviceResult.criticalIssues} critical`);

            } catch (error) {
                console.log(`   ❌ ${service.name}: Assessment failed - ${error.message}`);
        ...
        const averageScore = assessmentResults.length > 0 ? totalScore / assessmentResults.length : 0;

        // Determine overall grade
        let overallGrade = 'F';
        if (averageScore >= 90) overallGrade = 'A+ (Excellent)';
        else if (averageScore >= 85) overallGrade = 'A (Excellent)';
        else if (averageScore >= 80) overallGrade = 'B+ (Very Good)';
        else if (averageScore >= 75) overallGrade = 'B (Good)';
        else if (averageScore >= 70) overallGrade = 'C+ (Acceptable)';
        else if (averageScore >= 65) overallGrade = 'C (Acceptable)';
        else if (averageScore >= 60) overallGrade = 'D (Poor)';
        else overallGrade = 'F (Failed)';

        console.log(`\n📊 Cross-Browser Assessment Summary for ${browserName}:`);
        ...
        expect(averageScore, `${browserName} should have acceptable cross-browser compatibility`).toBeGreaterThan(40);
        expect(criticalIssues, `${browserName} should have no critical cross-browser issues`).toBe(0);

        // High priority browsers should have excellent compatibility
        if (browserName === 'chromium' || browserName === 'firefox' || browserName === 'webkit') {
            expect(averageScore, `High priority browser ${browserName} should have excellent compatibility`).toBeGreaterThan(70);
        }
    });
});
