...
class RealServiceTestRunner {
  ...
  async waitForServicesReady(maxWaitTime: number = 10000): Promise<void> {
    console.log('⏳ Checking services availability...');
    
    // Check if any services are available by testing port connectivity
    const portCheckPromises = Object.entries(SERVICES).map(async ([key, service]) => {
      try {
        ...
        return { key, available: response.status !== undefined };
      } catch (error) {
    ...
    const availableServices = portResults.filter(result => result.available).length;
    
    console.log(`📊 Port connectivity: ${availableServices}/${Object.keys(SERVICES).length} services available`);
    
    if (availableServices >= 3) {
      console.log('✅ Sufficient services are available for testing');
      return;
    }
    
    throw new Error(`Not enough services available: ${availableServices}/7 services responding`);
  }
}
...
describe('Gateway Service - Real Integration Tests', () => {
  ...
  describe('Gateway Health and Discovery', () => {
    test('should respond to gateway root endpoint', async () => {
      const response = await request(GATEWAY_BASE_URL)
        .get('/')
        .expect((res) => {
          // Gateway may return auth required (401) or redirect (302) or success (200)
          expect([200, 302, 401, 404]).toContain(res.status);
        });

      console.log(`✅ Gateway root endpoint - Status: ${response.status}`);
    });

    test('should handle gateway service discovery', async () => {
      const response = await request(GATEWAY_BASE_URL)
        .get('/api/gateway/services')
        .expect((res) => {
          // May require auth or return service list
          expect([200, 401, 404]).toContain(res.status);
        });

      if (response.status === 200) {
        expect(response.body).toHaveProperty('services');
        console.log(`✅ Service discovery successful - ${response.body.services?.length || 0} services`);
      } else {
        console.log(`ℹ️ Service discovery requires auth - Status: ${response.status}`);
      }
    });

    test('should provide gateway health information', async () => {
      const response = await request(GATEWAY_BASE_URL)
      ...
      if (response.status === 200) {
        ...
        console.log('✅ Gateway health endpoint accessible');
      } else {
        console.log(`ℹ️ Gateway health endpoint - Status: ${response.status}`);
      }
    });
  ...
  describe('Authentication and Authorization', () => {
    test('should handle requests without authentication', async () => {
      const response = await request(GATEWAY_BASE_URL)
        .get('/')
        .expect((res) => {
          // Gateway may allow access or require auth
          expect([200, 302, 401, 404]).toContain(res.status);
        });
    ...
    test('should handle protected API endpoints', async () => {
    ...
    test('should provide error details for unauthorized requests', async () => {
      const response = await request(GATEWAY_BASE_URL)
        .get('/api/gateway/services')
        .expect((res) => {
          expect([200, 401, 404]).toContain(res.status);
      ...
      if (response.status === 401) {
        ...
        console.log(`✅ Unauthorized error details - Code: ${response.body.code}`);
      } else {
        console.log(`ℹ️ Authorization test - Status: ${response.status}`);
      }
    });
